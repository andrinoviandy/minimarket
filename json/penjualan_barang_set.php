...
if (isset($_GET['start'])) {
	...
	if ($_GET['tgl1'] && $_GET['tgl2']) {
		if (isset($_GET['cari'])) {
			$sql = "select COUNT(DISTINCT barang_dijual_set.id) as jml from barang_dijual_set, pembeli where pembeli.id=barang_dijual_set.pembeli_id and (DATE_FORMAT(tgl_jual, '%d-%m-%Y') like '%$_GET[cari]%' or no_faktur_jual like '%$_GET[cari]%' or marketing like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%') and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]'";
		} else {
			$sql = "select COUNT(DISTINCT barang_dijual_set.id) as jml from barang_dijual_set, pembeli where pembeli.id=barang_dijual_set.pembeli_id and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]'";
		}
	} else {
		if (isset($_GET['cari'])) {
			$sql = "select COUNT(DISTINCT barang_dijual_set.id) as jml from barang_dijual_set, pembeli where pembeli.id=barang_dijual_set.pembeli_id and (DATE_FORMAT(tgl_jual, '%d-%m-%Y') like '%$_GET[cari]%' or no_faktur_jual like '%$_GET[cari]%' or marketing like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%')";
		} else {
			$sql = "select COUNT(DISTINCT barang_dijual_set.id) as jml from barang_dijual_set";
		}
	}
