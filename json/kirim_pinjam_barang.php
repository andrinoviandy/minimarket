...
if (isset($_GET['start'])) {
	...
	if ($_GET['tgl1'] && $_GET['tgl2']) {
		if (isset($_GET['cari'])) {
			$sql = "select COUNT(DISTINCT barang_pinjam_kirim.id) as jml from barang_pinjam_kirim,barang_pinjam_kirim_detail,barang_gudang,pembeli,barang_gudang_detail where barang_pinjam_kirim.id=barang_pinjam_kirim_detail.barang_pinjam_kirim_id and barang_gudang_detail.id=barang_pinjam_kirim_detail.barang_gudang_detail_id and barang_gudang.id=barang_gudang_detail.barang_gudang_id and pembeli.id=barang_pinjam_kirim.pembeli_id and (no_pengiriman like '%$_GET[cari]%' or nama_paket like '%$_GET[cari]%' or keterangan like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%' or nama_brg like '%$_GET[cari]%' or no_seri_brg like '%$_GET[cari]%') and tgl_kirim between '$_GET[tgl1]' and '$_GET[tgl2]'";
		} else {
			$sql = "select COUNT(DISTINCT barang_pinjam_kirim.id) as jml from barang_pinjam_kirim where tgl_kirim between '$_GET[tgl1]' and '$_GET[tgl2]'";
		}
	} else {
		if (isset($_GET['cari'])) {
			$sql = "select COUNT(DISTINCT barang_pinjam_kirim.id) as jml from barang_pinjam_kirim,barang_pinjam_kirim_detail,barang_gudang,pembeli,barang_gudang_detail where barang_pinjam_kirim.id=barang_pinjam_kirim_detail.barang_pinjam_kirim_id and barang_gudang_detail.id=barang_pinjam_kirim_detail.barang_gudang_detail_id and barang_gudang.id=barang_gudang_detail.barang_gudang_id and pembeli.id=barang_pinjam_kirim.pembeli_id and (no_pengiriman like '%$_GET[cari]%' or nama_paket like '%$_GET[cari]%' or keterangan like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%' or nama_brg like '%$_GET[cari]%' or no_seri_brg like '%$_GET[cari]%')";
		} else {
			$sql = "select COUNT(DISTINCT barang_pinjam_kirim.id) as jml from barang_pinjam_kirim";
		}
	}
