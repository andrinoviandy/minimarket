...
if (isset($_GET['start'])) {
	...
	//untuk jumlah
	if (isset($_GET['cari'])) {
		$sql = "select COUNT(*) as jml from stok_opname_detail_x where stok_opname_id = $_GET[id] and qrcode like '%$_GET[cari]%'";
	} else {
		$sql = "select COUNT(*) as jml from stok_opname_detail_x where stok_opname_id = $_GET[id]";
	}
