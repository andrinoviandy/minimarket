...
if (isset($_GET['start'])) {
	...
	//untuk jumlah
	if (isset($_GET['cari'])) {
		$sql = "select COUNT(DISTINCT tb_laporan_kerusakan_cs.id) as jml from from tb_laporan_kerusakan_cs where pembeli_id=$_GET[id] and (no_seri_brg like '%$_GET[cari]%' or kerusakan_alat like '%$_GET[cari]%' or DATE_FORMAT(tgl_input, '%d-%m-%Y') like '%$_GET[cari]%')";
	} else {
		$sql = "select COUNT(DISTINCT tb_laporan_kerusakan_cs.id) as jml from from tb_laporan_kerusakan_cs where pembeli_id=$_GET[id]";
	}
}
