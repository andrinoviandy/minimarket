...
$data = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_dijual,barang_dijual_qty,barang_gudang,barang_gudang_detail,pembeli,pemakai,alamat_provinsi,alamat_kabupaten,alamat_kecamatan where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and alamat_provinsi.id=pembeli.provinsi_id and alamat_kabupaten.id=pembeli.kabupaten_id and alamat_kecamatan.id=pembeli.kecamatan_id and pemakai.id=barang_dijual.pemakai_id and barang_dijual.id=".$_GET['id'].""));
?>
<html>
    <head>
...
Telp : <?php echo $data['kontak1_pemakai']." / ".$data['kontak2_pemakai']; ?></td>
  ...
  <?php 
  $q = mysqli_query($koneksi, "select *,barang_gudang.id as id_gudang from barang_dijual_qty,barang_dijual,barang_gudang where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and barang_dijual.id=".$_GET['id']." group by nama_brg"); 
  $no=0;
  ...
  <tr>
    ...
    <td align="center" valign="top"><?php echo number_format($d['harga_satuan'],0,',','.'); ?></td>
  ...
  <tr>
	...
	echo number_format($data['diskon_jual']/100*$t['total'],0,',','.'); } else {echo "-";} ?></td>
  </tr>
  <tr>
    <td colspan="4" align="right" valign="top"><strong>PPN (<?php echo $data['ppn_jual']."%"; ?>)</strong></td>
    <td align="right" valign="top"><?php 
	if ($data['ppn_jual']!=0) {
	echo number_format($data['ppn_jual']/100*$t['total'],0,',','.'); } else {echo "-";} ?></td>
  </tr>
  <tr>
    <td colspan="4" align="right" valign="top" style="padding:10px"><strong>TOTAL + PPN - Diskon</strong></td>
    <td align="right" valign="top" style="font-size:20px"><strong><?php echo "Rp".number_format(($t['total']-($t['total']*$data['diskon_jual']/100))+($t['total']*$data['ppn_jual']/100),0,',','.'); 
