...
if (isset($_POST['tambah'])) {
		if ($_SESSION['tgl_lapor']>$_POST['tgl_garansi_habis']) {
			$gr = "Habis";
			}
		else {
			$gr = "Masih";
			}
	$Result = mysqli_query($koneksi, "insert tb_laporan_kerusakan_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','$gr','".$_POST['id_kategori']."','".str_replace("\n","<br>",$_POST['problem'])."')");
	if ($Result) {
		echo "<script type='text/javascript'>
...
if (isset($_GET['simpan'])==1) {
	...
	$insert1=mysqli_query($koneksi, "insert into tb_laporan_kerusakan values('','".$_SESSION['id_akun']."','".$_SESSION['tgl_lapor']."','".$_SESSION['penelepon']."','".$_SESSION['nomor_hp']."','".$_SESSION['keluhan']."')");
	
	$id1 = mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as idd from tb_laporan_kerusakan"));
	$max_id = $id1['idd'];
	
	$q1 = mysqli_query($koneksi, "select * from tb_laporan_kerusakan_hash where akun_id=".$_SESSION['id']."");
	while ($d1 = mysqli_fetch_array($q1)) {
		$insert2 = mysqli_query($koneksi, "insert into tb_laporan_kerusakan_detail values('','$max_id','".$d1['alat_pelatihan_id']."','".$d1['status_garansi']."','".$d1['kategori_job_id']."','".$d1['problem']."')");
		}
		
	if ($insert1 and $insert2) {
			mysqli_query($koneksi, "delete from tb_laporan_kerusakan_hash");
	echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
...
<div class="content-wrapper">
  ...
  <thead>
    <tr>
      ...
      <td align="center" valign="bottom"><strong>No Seri/Nama Set     
      </strong>
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select * from tb_laporan_kerusakan_hash");
...
<center><a href="index.php?page=simpan_tambah_laporan&simpan=1"><button style="padding:10px" name="simpan_barang" class="btn btn-success" type="button"><span class="fa fa-check"></span> Simpan</button></a></center>
</div>
              </div>
            </div>
          </div>
  ...
  <div class="modal fade" id="modal-tambah">
        ...
        <?php 
		$q = mysqli_query($koneksi, "select *,alat_pelatihan.id as idd,pembeli_id as id_rumkit from barang_teknisi,barang_teknisi_detail,barang_teknisi_detail_teknisi, barang_dikirim,barang_dikirim_detail, barang_dijual, barang_gudang, barang_gudang_detail,pembeli, tb_teknisi,alat_uji_detail,alat_pelatihan where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dikirim_detail.barang_gudang_detail_id and barang_dijual.id=barang_dikirim.barang_dijual_id and barang_dikirim.id=barang_dikirim_detail.barang_dikirim_id and pembeli.id=barang_dijual.pembeli_id and barang_dikirim_detail.id=barang_teknisi_detail.barang_dikirim_detail_id and barang_teknisi.id=barang_teknisi_detail.barang_teknisi_id and barang_teknisi_detail.id=barang_teknisi_detail_teknisi.barang_teknisi_detail_id and tb_teknisi.id=barang_teknisi_detail_teknisi.teknisi_id and barang_teknisi_detail.id=alat_uji_detail.barang_teknisi_detail_id and alat_uji_detail.id=alat_pelatihan.alat_uji_detail_id and pembeli.id=".$_SESSION['id_akun']."");
