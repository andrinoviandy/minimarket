...
<div class="content-wrapper">
  ...
  <tr>
    ...
    <?php } else if ($data['nama_transaksi']=='Pembayaran Piutang Alkes Ber No Seri') { 
	$dd = mysqli_fetch_array(mysqli_query($koneksi, "select utang_piutang_id from utang_piutang_bayar where keuangan_id=$data[idd]"));
	?>
    <a href="index.php?page=bayar_piutang&id=<?php echo $dd['utang_piutang_id']; ?>">
    <?php } else if ($data['nama_transaksi']=='Pembayaran Piutang Aksesoris') { 
	$dd = mysqli_fetch_array(mysqli_query($koneksi, "select utang_piutang_aksesoris_id from utang_piutang_aksesoris_bayar where keuangan_id=$data[idd]"));
	?>
    <a href="index.php?page=bayar_piutang_aksesoris&id=<?php echo $dd['utang_piutang_aksesoris_id']; ?>">
  ...
  <tfoot>
    ...
    <strong>
	<?php 
	 $kredit = mysqli_fetch_array(mysqli_query($koneksi, "select sum(saldo) as total from keuangan,keuangan_detail where keuangan.id=keuangan_detail.keuangan_id and keuangan_detail.coa_sub_id=".$_GET['id']." and db_cr='db' order by tgl_transaksi DESC,keuangan.id DESC"));
    ...
    <td>
      ...
      <strong>
        <?php 
	 $debet = mysqli_fetch_array(mysqli_query($koneksi, "select sum(saldo) as total from keuangan,keuangan_detail where keuangan.id=keuangan_detail.keuangan_id and keuangan_detail.coa_sub_id=".$_GET['id']." and db_cr='cr' order by tgl_transaksi DESC,keuangan.id DESC"));
    ...
    <td>
      ...
      <strong style="font-size:18px">
        <?php echo "Rp".number_format($kredit['total']-$debet['total'],2,',','.'); ?>
        </strong>
    </td>
  </tfoot>
</table><br />
