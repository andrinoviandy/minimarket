<?php
$data = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_dijual_inventory where id=".$_GET['id'].""));
...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	$cek_harga = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_inventory where id=".$_POST['id_akse'].""));
	...
	$simpan = mysqli_query($koneksi, "insert into barang_dijual_inventory_qty values('','".$_GET['id']."','".$_POST['id_akse']."','".$sel_hrg_jual['harga_satuan']."','".$_POST['qty']."','')");
	if ($simpan) {
		$jml = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual*harga_jual_saat_itu) as total from barang_dijual_inventory_qty where barang_dijual_inventory_id=".$_GET['id'].""));
		  mysqli_query($koneksi, "update barang_dijual_inventory set total_harga=$jml[total]+($jml[total]*ppn_jual/100)-($jml[total]*diskon_jual/100) where id=".$_GET['id']."");
		$update_piutang = mysqli_query($koneksi, "update utang_piutang_inventory set nominal=$jml[total] where no_faktur_no_po_inven='".$data['no_po_jual']."'");
		echo "<script>window.location='index.php?page=ubah_jual_inventory_uang&id=$_GET[id]'</script>";
		}
	/*} else {
...
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_dijual_inventory_qty where id=".$_GET['id_hapus']."");
	$jml = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual*harga_jual_saat_itu) as total from barang_dijual_inventory_qty where barang_dijual_inventory_id=".$_GET['id'].""));
		  mysqli_query($koneksi, "update barang_dijual_inventory set total_harga=".$jml['total']." where id=".$_GET['id']."");
		  $update_piutang = mysqli_query($koneksi, "update utang_piutang_inventory set nominal=$jml[total] where no_faktur_no_po='".$data['no_po_jual']."'");
	}
?>
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
      Ubah Penjualan Barang Inventori</h1><ol class="breadcrumb">
...
";
        ...
        <?php
		$stok_po1 = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual) as stok_po from barang_dijual_inventory_qty where barang_inventory_id=".$d['id']." and status_kirim=0"));
		 
		$jsArray .= "dtBrg['" . $d['id'] . "'] = {tipe_akse:'".addslashes($d['tipe_brg'])."',
						merk_akse:'".addslashes($d['merk_brg'])."',
						stok_total:'".addslashes($d['stok_total']-($stok_po1['stok_po']))."',
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_dijual_inventory_qty.id as idd from barang_dijual_inventory_qty,barang_inventory where barang_inventory.id=barang_dijual_inventory_qty.barang_inventory_id and barang_dijual_inventory_id=".$_GET['id']."");
  ...
  <?php }} else {
    ...
    <tr>
      ...
      <td><?php
      $total1 = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual*harga_satuan) as total1 from barang_inventory,barang_dijual_inventory_qty where barang_inventory.id=barang_dijual_inventory_qty.barang_inventory_id and barang_dijual_inventory_id=".$_GET['id'].""));
...
<?php 
  if (isset($_POST['ubah_qty'])) {
	$Result = mysqli_query($koneksi, "update barang_dijual_inventory_qty set qty_jual=".$_POST['qty']." where id=".$_GET['id_ubah']."");
	if ($Result) {
		$jml = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual*harga_jual_saat_itu) as total from barang_dijual_inventory_qty where barang_dijual_inventory_id=".$_GET['id'].""));
		  mysqli_query($koneksi, "update barang_dijual_inventory set total_harga=$jml[total]+($jml[total]*ppn_jual/100)-($jml[total]*diskon_jual/100) where id=".$_GET['id']."");
		echo "<script type='text/javascript'>
		alert('Data Berhasil Di Ubah !');
		window.location='index.php?page=ubah_jual_inventory_uang&id=$_GET[id]';
		</script>";
		}
	}
		?>
