...
if (isset($_GET['simpan_barang'])==1) {
	$simpan1 = mysqli_query($koneksi, "insert into aksesoris_kirim values('','".$_SESSION['nama_paket']."','".$_SESSION['no_pengiriman']."','".$_SESSION['tgl_pengiriman']."','".$_SESSION['no_po']."','0000-00-00')");
	$max = mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_max from aksesoris_kirim"));
	$jml = count($_POST['p']);
	for ($i=0; $i<$jml; $i++) {
		$simpan2 = mysqli_query($koneksi, "insert into aksesoris_kirim_detail values('','".$max['id_max']."','".$_POST['p'][$i]."')");
		mysqli_query($koneksi, "update aksesoris_jual_detail set status_kirim_akse=1 where id=".$_POST['p'][$i]."");
		}
...
<div class="content-wrapper">
  ...
  <tr>
    ...
    <td><?php echo $_SESSION['no_po']; ?></td>
                ...
                <h3 align="left">
                  Pilih Aksesoris Yang Akan Dikirim
                </h3>
                ...
                <br />
                <form method="post" action="index.php?page=simpan_kirim_aksesoris&simpan_barang=1">
                <table width="100%" id="" class="table table-bordered table-hover">
  <thead>
    <tr>
  ...
  $no=0;
  $q_akse2 = mysqli_query($koneksi, "select *, aksesoris_jual_detail.id as idd from aksesoris,aksesoris_detail,aksesoris_jual,aksesoris_jual_detail where aksesoris.id=aksesoris_detail.aksesoris_id and aksesoris_detail.id=aksesoris_jual_detail.aksesoris_detail_id and aksesoris_jual.id=aksesoris_jual_detail.aksesoris_jual_id and aksesoris_jual_detail.status_kirim_akse=0 and aksesoris_jual.id=$_GET[id]");
  ...
  <tr>
    ...
    <td align="center"><?php echo "Rp ".number_format($data_akse['harga_akse'],0,',',',').".00"; ?></td>
    <td align="center"><input name="p[]" checked="checked" type="checkbox" value="<?php echo $data_akse['idd']; ?>" style="height:20px; width:20px" /></td>
    </tr>
    <?php } ?>
</table>
<br />
<center><a href="index.php?page=simpan_kirim_aksesoris&simpan_barang=1"><button name="simpan_barang" class="btn btn-success" type="submit"><span class="fa fa-plus"></span> Simpan</button></a>&nbsp;&nbsp;<a href="index.php?page=penjualan_aksesoris"><button name="batal" class="btn btn-success" type="button"><span class="fa  fa-times-circle"></span> Batal</button></a></center>
</form>
</div>
              </div>
            </div>
