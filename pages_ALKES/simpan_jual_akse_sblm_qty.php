...
if (isset($_GET['simpan_barang'])==1) {
	...
	while ($d2 = mysqli_fetch_array($q2)) {
		$simpan2=mysqli_query($koneksi, "insert into aksesoris_jual_detail values('','$id_jual','".$d2['aksesoris_detail_id']."','0')");
	...
	window.location='index.php?page=penjualan_aksesoris'</script>";
		}
	}


if (isset($_POST['simpan_tambah_aksesoris'])) {
	$no=mysqli_num_rows(mysqli_query($koneksi, "select * from aksesoris_jual_hash"))+1;
	$simpan = mysqli_query($koneksi, "insert into aksesoris_jual_hash values('','".$_SESSION['id']."','".$_POST['no_seri']."')");
	if ($simpan) {
...
";
    ...
    <?php 
	$q_seri = mysqli_query($koneksi, "select *,aksesoris_detail.id as idd from aksesoris_detail INNER JOIN aksesoris ON aksesoris.id=aksesoris_detail.aksesoris_id order by no_seri_akse ASC");
	while ($d_seri=mysqli_fetch_array($q_seri)) {
	?>
    <option id="no_seri" value="<?php echo $d_seri['idd']; ?>" class="<?php echo $d_seri['aksesoris_id']; ?>" <?php if ($d_seri['status_terjual_akse']==1) { echo "disabled"; } ?>><?php echo $d_seri['no_seri_akse']; ?></option>
    <?php } ?>
    </select>
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,aksesoris_jual_hash.id as idd from aksesoris_jual_hash,aksesoris_detail,aksesoris where aksesoris.id=aksesoris_detail.aksesoris_id and aksesoris_detail.id=aksesoris_jual_hash.aksesoris_detail_id");
  ...
  <tr>
    ...
    <td align="center"><?php echo $data_akse['no_seri_akse']; ?></td>
...
<center><a href="index.php?page=simpan_jual_akse&simpan_barang=1"><button name="simpan_barang" class="btn btn-success" type="submit"><span class="fa fa-plus"></span> Simpan</button></a>&nbsp;&nbsp;<a href="index.php?page=penjualan_aksesoris"><button name="batal" class="btn btn-success" type="submit"><span class="fa  fa-times-circle"></span> Batal</button></a></center>
</div>
              </div>
            </div>
          </div>
