...
if (isset($_POST['simpan_barang'])) {
	echo "<script type='text/javascript'>		window.location='index.php?page=simpan_tambah_barang_set44&id=$_GET[id]&stok_set=$_GET[stok_set]&id_gudang=$_GET[id_gudang]&id_detail=$_GET[id_detail]';
		</script>
		";
...
<div class="content-wrapper">
  ...
  <tr>
    ...
    <td><input id="" name="no_po" class="form-control" type="text"  placeholder="" size="4" required value="<?php $no_po=mysqli_fetch_array(mysqli_query($koneksi, "select no_po_pesan from barang_pesan_set where id=$_GET[id]"));
	echo $no_po['no_po_pesan']; ?>"/></td>
    <td><input name="stok_set" class="form-control" type="text"  placeholder="" size="4" required="required" value="<?php echo $_GET['stok_set']; ?>"/></td>
    <td>
    <?php
    $da = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_gudang_set,barang_gudang_po_set,barang_gudang_set_1,barang_gudang_set_2 where barang_gudang_set.id=barang_gudang_po_set.barang_gudang_set_id and barang_gudang_po_set.id=barang_gudang_set_1.barang_gudang_po_set_id and barang_gudang_set_1.id=barang_gudang_set_2.barang_gudang_set1_id and barang_gudang_set.id=".$_GET['id'].""));
	?>
    <input name="stok_dalam_1set" class="form-control" type="text" readonly="readonly" value="<?php echo $da; ?>" placeholder="" size="4" required="required"/></td>
    </tr>
</table><br /><center>
