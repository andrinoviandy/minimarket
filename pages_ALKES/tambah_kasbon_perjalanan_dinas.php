...
if (isset($_POST['simpan_barang'])) {
	...
	while ($d2 = mysqli_fetch_array($q2)) {
		...
		$cek = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_teknisi_detail where barang_teknisi_id=".$d2['barang_teknisi_id']." and status_teknisi=1"));
		if ($cek==0) {
...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	...
	$simpan = mysqli_query($koneksi, "insert into kasbon_perjalanan_dinas_detail_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','".$_POST['id_teknisi']."','".$_POST['estimasi_berangkat']."','".$_POST['tgl_berangkat']."','".$_POST['deskripsi']."')");
	if ($simpan) {
		echo "<script>window.location='index.php?page=tambah_kasbon_perjalanan_dinas'</script>";
		}
...
if (isset($_POST['simpan_tambah_aksesoris2'])) {
	...
	$simpan = mysqli_query($koneksi, "insert into kasbon_perjalanan_dinas_detail_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','".$cek['teknisi_id']."','".$cek['estimasi']."','".$cek['tgl_berangkat_teknisi']."','".$cek['deskripsi']."')");
	if ($simpan) {
		echo "<script>window.location='index.php?page=tambah_kasbon_perjalanan_dinas'</script>";
		}
...
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from kasbon_perjalanan_dinas_detail_hash where id=".$_GET['id_hapus']."");
	}
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
      Tambah Data Kasbon (Perjalanan Dinas)</h1><ol class="breadcrumb">
...
<button name="tambah_laporan" class="btn btn-success pull pull-right" type="button" data-toggle="modal" data-target="#modal-pilihnoseri" style="margin-right:20px"><span class="fa fa-plus"></span> Tambah SPI</button>
<br /><br />
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,kasbon_perjalanan_dinas_detail_hash.id as idd from kasbon_perjalanan_dinas_detail_hash,barang_teknisi,barang_teknisi_detail,barang_dikirim_detail,barang_dikirim,barang_dijual,tb_teknisi,pembeli where barang_teknisi.id=kasbon_perjalanan_dinas_detail_hash.barang_teknisi_id and barang_teknisi.id=barang_teknisi_detail.barang_teknisi_id and barang_dikirim_detail.id=barang_teknisi_detail.barang_dikirim_detail_id and barang_dikirim.id=barang_dikirim_detail.barang_dikirim_id and barang_dijual.id=barang_dikirim.barang_dijual_id and tb_teknisi.id=kasbon_perjalanan_dinas_detail_hash.teknisi_id and pembeli.id=barang_dijual.pembeli_id and admin_id=".$_SESSION['id']." group by kasbon_perjalanan_dinas_detail_hash.id order by kasbon_perjalanan_dinas_detail_hash.id ASC");
  ...
  <tr>
    ...
    <td align="center"><a href="index.php?page=tambah_kasbon_perjalanan_dinas&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
...
<center><button name="simpan_barang" class="btn btn-success" data-toggle="modal" data-target="#modal-tambah" <?php if ($jm==0) {echo "disabled"; }?>><span class="fa fa-check"></span> Next</button>&nbsp;&nbsp;<a href="index.php?page=kasbon_perjalanan_dinas"><button name="batal" class="btn btn-success" type="submit"><span class="fa  fa-times-circle"></span> Batal</button></a></center>
</div>
              </div>
            </div>
          </div>
