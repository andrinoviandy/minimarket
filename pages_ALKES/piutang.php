...
?>
  ...
  <tr>
    ...
    <td>
    <?php 
	$dd = mysqli_fetch_array(mysqli_query($koneksi, "select no_kontrak from barang_dijual where no_po_jual='".$json[$i]['no_faktur_no_po']."'"));
	echo $dd['no_kontrak']; ?>
    </td>
    <td><a href="#" data-toggle="modal" data-target="#modal-detailbarang<?php echo $json[$i]['idd']; ?>"><small data-toggle="tooltip" title="Detail Barang" class="label bg-primary"><span class="fa fa-folder-open"></span></small></a></td>
    <td><?php echo $json[$i]['klien']; ?></td>
      ...
      <?php if ($json[$i]['status_lunas']==0){if ($t_b['total']==0){$b="btn-danger";} else {$b="btn-warning";}} else {$b="btn-success";} ?>
      <td class="<?php echo $b; ?>" align="center"><?php if ($json[$i]['status_lunas']==0){echo "Belum Lunas";} else {echo "Sudah Lunas";} ?></td>
    ...
    <td>
      ...
      <a href="index.php?page=piutang&id_hapus=<?php echo $json[$i]['idd']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
      <a href="index.php?page=ubah_piutang&id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a><br />-->
      <?php //} ?>
	  <?php if ($json[$i]['status_lunas']==0) { ?>
...
if (isset($_GET['tgl1']) and isset($_GET['tgl2'])) {
	$queryy = mysqli_query($koneksi, "select *,utang_piutang.id as idd from barang_dijual,barang_dijual_qty,barang_gudang,pembeli,utang_piutang where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.no_po_jual=utang_piutang.no_faktur_no_po and u_p='Piutang' and tgl_input between '$_GET[tgl1]' and '$_GET[tgl2]' group by utang_piutang.id order by tgl_input DESC, utang_piutang.id DESC");

}
elseif (isset($_GET['kunci']) and isset($_GET['pilihan'])) {
	$queryy = mysqli_query($koneksi, "select *,utang_piutang.id as idd from barang_dijual,barang_dijual_qty,barang_gudang,pembeli,utang_piutang where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.no_po_jual=utang_piutang.no_faktur_no_po and u_p='Piutang' and $_GET[pilihan] like '%$_GET[kunci]%' group by utang_piutang.id order by tgl_input DESC, utang_piutang.id DESC");

} else {
	$queryy = mysqli_query($koneksi, "select *,utang_piutang.id as idd from utang_piutang where u_p='Piutang' order by tgl_input DESC, utang_piutang.id DESC");
	}
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <option value="nama_pembeli">Berdasarkan Nama Klien</option>
