...
$q1=mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_pinjam_kirim.id as idd from barang_pinjam_kirim where id=".$_GET['id'].""));

if (isset($_POST['tambah_laporan'])) {
	$Result = mysqli_query($koneksi, "update barang_pinjam_kirim set nama_paket='".$_POST['nama_paket']."', no_pengiriman='".$_POST['no_pengiriman']."', tgl_kirim='".$_POST['tgl_kirim']."', ekspedisi='".$_POST['ekspedisi']."', via_pengiriman='".$_POST['via_kirim']."', estimasi_barang_sampai='".$_POST['estimasi']."', biaya_pengiriman='".$_POST['biaya_jasa']."', tgl_sampai='".$_POST['tgl_sampai']."',keterangan='".str_replace("\n","<br>",$_POST['keterangan'])."' where id=".$_GET['id']."");
	if ($Result) {
		echo "<script type='text/javascript'>
		alert('Data Berhasil Di Ubah !');
		window.location='index.php?page=kirim_pinjam_barang';
		</script>";
		}
	}
?>
          ...
          <div class="box box-warning"><!-- /.chat -->
            ...
            <div class="box-header with-border">
              ...
              <option value="">...</option>
					...
					?>
					<option <?php if ($q1['pembeli_id']==$row['idd']) {echo "selected";} ?> value="<?php echo $row['idd'];?>"><?php echo $row['nama_pembeli'];?></option>    
					<?php	
					}      
					?>    
				</select>
