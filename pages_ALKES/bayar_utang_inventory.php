<?php
$data=mysqli_fetch_array(mysqli_query($koneksi, "select *,utang_piutang_inventory.id as idd from utang_piutang_inventory where id=$_GET[id]"));
...
if (isset($_POST['tambah_header'])) {
	...
	if ($cek_saldo['saldo']<(str_replace(".","",$_POST['nominal']))) {
		...
		$simpan_keuangan = mysqli_query($koneksi,"insert into keuangan values('','".$_POST['tgl_input']."','Pembayaran Hutang Inventory','".$_POST['deskripsi']."','".str_replace(".","",$_POST['nominal'])."')");
	$max = mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_max from keuangan"));
	$simpan_keuangan_detail1 = mysqli_query($koneksi,"insert into keuangan_detail values('','$max[id_max]','2','9','45','cr')");
	
	$Result = mysqli_query($koneksi, "insert into utang_piutang_inventory_bayar values('','$max[id_max]','Hutang','$_GET[id]','".$_POST['tgl_input']."','".str_replace(".","",$_POST['nominal'])."','".$_POST['deskripsi']."','".$_POST['akun']."')");
	if ($Result) {
		...
		if ($sel['jumlah']>=$data['nominal']) {
			mysqli_query($koneksi, "update utang_piutang_inventory,barang_pesan_inventory set utang_piutang_inventory.status_lunas=1,barang_pesan_inventory.status_lunas=1 where barang_pesan_inventory.no_po_pesan=utang_piutang_inventory.no_faktur_no_po_inven and utang_piutang_inventory.id=".$_GET['id']."");
			}
			echo "<script type='text/javascript'>
		alert('Pembayaran Berhasil Disimpan !');
		window.location='index.php?page=bayar_utang_inventory&id=$_GET[id]'
		</script>";
		}
	}
}
if (isset($_GET['id_hapus'])) {
	...
	$up = mysqli_query($koneksi, "update utang_piutang_inventory,barang_pesan_inventory set utang_piutang_inventory.status_lunas=0,barang_pesan_inventory.status_lunas=0 where barang_pesan_inventory.no_po_pesan=utang_piutang_inventory.no_faktur_no_po_inven and utang_piutang_inventory.id=$_GET[id]");
		}
		}
	}
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1><span class="active">Bayar Hutang Inventory</span></h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Hutang</li>
        <li class="active">Bayar Hutang Inventory</li>
      </ol>
    </section>
      ...
      <section class="col-lg-4 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            <div class="box-footer">
              <div class="box-body">
              <a href="index.php?page=utang_inventory"><button name="tambah_header" class="btn btn-success" type="button"> Kembali Ke Halaman Sebelumnya </button></a>
<center>
...
<?php if ($data['status_lunas']==0){$b="#FF0000";} else {$b="#00CC66";} ?>
  ...
  <?php 
  $q2=mysqli_query($koneksi, "select *,utang_piutang_inventory_bayar.id as idd from utang_piutang_inventory_bayar,buku_kas where buku_kas.id=utang_piutang_inventory_bayar.buku_kas_id and u_p='Hutang' and utang_piutang_inventory_id=$_GET[id]");
  ...
  <tr>
    ...
    <td><?php echo "Rp ".number_format($d['nominal'],2,',','.'); ?>
      ...
      <td><a href="index.php?page=bayar_utang_inventory&id_hapus=<?php echo $d['idd']; ?>&id=<?php echo $_GET['id']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Riwayat Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
        <!--<a href="index.php?page=bayar_utang_inventory&id_ubah=<?php echo $d['idd']; ?>&id=<?php echo $_GET['id']; ?>#openUbah"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a>--></td>
    </tr>
 <?php } ?>
</table>  
			  <script type="text/javascript">
function yesnoCheck() {
  ...
  <?php
  $da = mysqli_fetch_array(mysqli_query($koneksi, "select *,buku_kas.id as idd from utang_piutang_bayar,buku_kas where buku_kas.id=utang_piutang_bayar.buku_kas_id and utang_piutang_bayar.id=$_GET[id_ubah]"));
...
<div class="modal fade" id="modal-detail<?php echo $data['idd']; ?>">
                        ...
                        <div class="box-body table-responsive no-padding">
                          <div class="">
                            ...
                            <div class="table-responsive no-padding">
                              <?php
              $dataa = mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_pesan_inventory.id as idd from barang_pesan_inventory,principle,mata_uang where mata_uang.id=barang_pesan_inventory.mata_uang_id and principle.id=barang_pesan_inventory.principle_id and no_po_pesan='".$data['no_faktur_no_po_inven']."'"));
			  ?>
                              <table width="100%" id="table" class="table table-bordered text-nowrap">
                                <thead>
                                  <tr>
                                ...
                                <tr>
                                  <td><?php echo date("d F Y",strtotime($dataa['tgl_po_pesan'])); ?></td>
                            ...
                            <table width="100%" id="example2" class="table table-bordered table-hover">
                              <thead>
                                <tr>
                                  ...
                                  <td align="center" valign="bottom"><strong>Merk </strong></td>
                                  <td align="center" valign="bottom"><strong>Qty</strong></td>
                                  <td align="center" valign="bottom"><strong>Mata Uang </strong></td>
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_inventory_detail.id as idd,barang_inventory.id as id_gudang from barang_pesan_inventory_detail,barang_inventory,mata_uang where barang_inventory.id=barang_pesan_inventory_detail.barang_inventory_id and mata_uang.id=barang_pesan_inventory_detail.mata_uang_id and barang_pesan_inventory_detail.barang_pesan_inventory_id=$dataa[idd]");
  ...
  ?>
	...
	echo $data_akse['diskon']." %";
		...
		echo "0 %";
                              ...
                              <tr>
                                ...
                                <td align="right"><?php
    $total = mysqli_fetch_array(mysqli_query($koneksi, "select *,sum(harga_total) as total from barang_pesan_inventory_detail where barang_pesan_inventory_id=$dataa[idd]"));
        ...
        ?>
                                  <?php echo $dataa['simbol']." ".number_format($total['total'],2,',','.'); ?></td>
                                <td></td>
                              </tr>
                              <tr>
                                <td colspan="8" align="right" valign="bottom"><strong>Total Price + PPN(<?php echo $dataa['ppn']."%"; ?>) =</strong></td>
                                <td align="right"><?php echo $dataa['simbol']." ".number_format(($total['total'])+(($total['total'])*floatval($data['ppn'])/100),2,',','.'); ?></td>
                                <td></td>
                              </tr>
                              <tr>
	...
	echo $dataa['simbol']." ".number_format($dataa['cost_byair'],2,',','.');} else {echo $dataa['simbol']." "."0";} ?></td>
                              ...
                              <tr>
                                ...
                                <td align="right" valign="top"><?php
      $mu= mysqli_fetch_array(mysqli_query($koneksi, " select * from utang_piutang_inventory where no_faktur_no_po_inven='".$dataa['no_po_pesan']."'"));
	  if ($mu['nominal']!=0) {
