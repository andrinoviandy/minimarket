...
if (isset($_POST['tambah_header'])) {
	...
	$ce = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_pesan_akse_detail_hash where akun_id=".$_SESSION['id'].""));
	if ($ce!=0) {
	...
	$simpan1=mysqli_query($koneksi, "insert into barang_pesan_akse values('','".$_SESSION['tgl_po']."','".$_SESSION['no_po']."','Dalam Negeri','$max_id_princ','".$_SESSION['ppn']."','".$_SESSION['cara_pembayaran']."','".$_SESSION['mata_uang']."','".$almat_peng."','".$_SESSION['jalur_pengiriman']."','".$_SESSION['estimasi_pengiriman']."','".$_SESSION['catatan']."','0','','".$_POST['total_price']."','".$_POST['total_price_ppn']."','".$_POST['cost_byair']."','".$_POST['cost_cf']."','','','')");
	$d1=mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_pesan from barang_pesan_akse"));
	$id_pesan=$d1['id_pesan'];
	//simpan barang pesan detail
	$q2=mysqli_query($koneksi, "select * from barang_pesan_akse_detail_hash where akun_id=".$_SESSION['id']."");
	while ($d2 = mysqli_fetch_array($q2)) {
		
		$simpan2=mysqli_query($koneksi, "insert into barang_pesan_akse_detail values('','$id_pesan','".$d2['aksesoris_id']."','".$d2['qty']."','".$d2['mata_uang_id']."','".$d2['harga_perunit']."','".$d2['diskon']."','".$d2['harga_total']."','".$d2['catatan_spek']."','0')");
			
		}
		
	if ($simpan1 and $simpan2) {
		$Result = mysqli_query($koneksi, "insert into utang_piutang_aksesoris values('','Hutang','".$_SESSION['no_po']."','".$_POST['tgl_input']."','".$_POST['jatuh_tempo']."','".$_POST['nominal']."','".$_POST['klien']."','".$_POST['deskripsi']."','0')");
		mysqli_query($koneksi, "delete from barang_pesan_akse_detail_hash where akun_id=".$_SESSION['id']."");
	echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=pembelian_akse'</script>";
		}
	}
	else {
		echo "<script type='text/javascript'>
	alert('Silkan Tambah Data Terlebih Dahulu !');
	window.location='index.php?page=simpan_tambah_pemesanan_akse'</script>";
		}
}

if (isset($_POST['simpan_tambah_aksesoris'])) {
	$no=mysqli_num_rows(mysqli_query($koneksi, "select * from barang_pesan_akse_detail_hash"))+1;
	$simpan = mysqli_query($koneksi, "insert into barang_pesan_akse_detail_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','".$_POST['qty']."','".$_SESSION['mata_uang']."','".$_POST['harga_perunit']."','".$_POST['diskon']."','".$_POST['total_harga']."','".$_POST['catatan_spek']."')");
	echo "<script>window.location='index.php?page=simpan_tambah_pemesanan_akse'</script>";
	}
	
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_pesan_akse_detail_hash where id=".$_GET['id_hapus']."");
	}
?>
...
<div class="content-wrapper">
  ...
  <tr>
    ...
    <td><?php echo $_SESSION['catatan']; ?></td>
                ...
                <h3 align="left">
                Tambah <strong>Aksesoris</strong> </h3>
  ...
  <tr>
    ...
    <td><select name="id_akse" id="id_akse" class="form-control select2" autofocus="autofocus" required="required" onchange="changeValue(this.value)">
...
";
      ...
      <option value="<?php echo $d['id']; ?>"><?php echo $d['nama_akse']." - ".$d['tipe_brg']; ?></option>
      <?php 
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_akse_detail_hash.id as idd,aksesoris.id as id_gudang from barang_pesan_akse_detail_hash,aksesoris,mata_uang where aksesoris.id=barang_pesan_akse_detail_hash.aksesoris_id and mata_uang.id=barang_pesan_akse_detail_hash.mata_uang_id and akun_id=".$_SESSION['id']."");
  ...
  <tr>
    ...
    <td align="center"><a href="index.php?page=simpan_tambah_pemesanan_akse&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
    ...
    <td align="center">
    <?php
    $total = mysqli_fetch_array(mysqli_query($koneksi, "select sum(harga_total) as total from barang_pesan_akse_detail_hash"));
...
<center><a href="index.php?page=simpan_tambah_pemesanan_akse#open_utang"><button name="simpan_barang" class="btn btn-success" type="button"><span class="fa fa-plus"></span> Simpan</button></a>&nbsp;&nbsp;<a href="index.php?page=pembelian_akse"><button name="batal" class="btn btn-success" type="button"><span class="fa  fa-times-circle"></span> Batal</button></a></center>
              </div>
              </div>
            </div>
          </div>
