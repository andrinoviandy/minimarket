<?php
$data=mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_dikirim_inventory.id as idd from barang_dikirim_inventory where id=$_GET[id]"));
...
if (isset($_POST['tambah_header'])) {
	...
	if ($cek_saldo['saldo']<$_POST['nominal']) {
		...
		$simpan_keuangan = mysqli_query($koneksi,"insert into keuangan values('','".$_POST['tgl_input']."','Biaya Ekspedisi Inventory','".$_POST['deskripsi']."','".$_POST['nominal']."')");
	...
	$Result = mysqli_query($koneksi, "insert into bayar_ekspedisi_inventory values('','$max[id_max]','$_GET[id]','".$_POST['tgl_input']."','".$_POST['nominal']."','".$_POST['deskripsi']."','".$_POST['akun']."')");
	if ($Result) {
...
if (isset($_GET['id_hapus'])) {
	$sel = mysqli_fetch_array(mysqli_query($koneksi, "select * from bayar_ekspedisi_inventory where id=$_GET[id_hapus]"));
	$up = mysqli_query($koneksi, "update bayar_ekspedisi_inventory,buku_kas set saldo=saldo+$sel[nominal] where buku_kas.id=bayar_ekspedisi_inventory.buku_kas_id and bayar_ekspedisi_inventory.buku_kas_id=".$sel['buku_kas_id']."");
	if ($up) { 
	...
	$de = mysqli_query($koneksi, "delete from bayar_ekspedisi_inventory where id=$_GET[id_hapus]");
	}
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1><span class="active">Bayar Biaya Ekspedisi Inventory</span></h1>
      <ol class="breadcrumb">
      ...
      <section class="col-lg-4 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            <div class="box-footer">
              <div class="box-body">
              <a href="index.php?page=utang_ekspedisi_inventory"><button name="tambah_header" class="btn btn-success" type="button"> Kembali Ke Halaman Sebelumnya </button></a>
<center>
...
function yesnoCheck() {
  ...
  <tr>
  <?php
    $jumlah_bayar = mysqli_fetch_array(mysqli_query($koneksi, "select sum(nominal) as total from bayar_ekspedisi_inventory where barang_dikirim_inventory_id=$_GET[id]"));
    ...
    <td>
    <?php echo date("d M Y",strtotime($data['tgl_kirim']));  ?>
  </td>
  ...
  <?php 
  $q2=mysqli_query($koneksi, "select *,bayar_ekspedisi_inventory.id as idd from bayar_ekspedisi_inventory,buku_kas where buku_kas.id=bayar_ekspedisi_inventory.buku_kas_id and barang_dikirim_inventory_id=$_GET[id]");
  ...
  <tr>
    ...
    <td><?php echo "Rp ".number_format($d['nominal'],2,',','.'); ?>
      ...
      <td><a href="index.php?page=bayar_ekspedisi_inventory&id_hapus=<?php echo $d['idd']; ?>&id=<?php echo $_GET['id']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Riwayat Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
 <?php } ?>
</table>  
			  <script type="text/javascript">
