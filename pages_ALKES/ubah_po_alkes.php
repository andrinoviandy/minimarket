...
if (isset($_POST['simpan_barang'])) {
	$simpan = mysqli_query($koneksi, "update barang_pesan set total_price='".$_POST['total_price']."', total_price_ppn='".$_POST['total_price_ppn']."', cost_byair='".$_POST['cost_byair']."', cost_cf='".$_POST['cost_cf']."' where id=$_GET[id]");		
	if ($simpan) {
...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	...
	$simpan = mysqli_query($koneksi, "update barang_pesan_detail set barang_gudang_id='".$_POST['id_akse']."', qty='".$_POST['qty']."', mata_uang_id='".$_POST['mata_uang']."',harga_perunit='".$_POST['harga_perunit']."', diskon='".$_POST['diskon']."', harga_total='".$_POST['total_harga']."',catatan_spek='".$_POST['catatan_spek']."' where id=$_GET[id_ubah]");
	if ($simpan) {
	$sel = mysqli_fetch_array(mysqli_query($koneksi, "select *, sum(harga_total) as total from barang_pesan_detail where barang_pesan_id=".$_GET['id'].""));
	if ($sel) {
		$tpp=$sel['total']+($sel['total']*$data['ppn']/100);
	mysqli_query($koneksi, "update barang_pesan set total_price=".$sel['total'].", total_price_ppn=".$tpp." where id=".$_GET['id']."");
	echo "<script type='text/javascript'>
...
<div class="content-wrapper">
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_detail.id as idd,barang_gudang.id as id_gudang from barang_pesan_detail,barang_gudang,mata_uang where barang_gudang.id=barang_pesan_detail.barang_gudang_id and mata_uang.id=barang_pesan_detail.mata_uang_id and barang_pesan_detail.id=$_GET[id_ubah]");
...
";
        ...
        <option <?php if ($data_akse['id_gudang']==$d['id']) { echo "selected";} ?> value="<?php echo $d['id']; ?>"><?php echo $d['nama_brg']." - ".$d['tipe_brg']; ?></option>
        <?php 
    ...
    <td align="center"><input id="tipe_akse" name="tipe_akse" class="form-control" type="text" placeholder="Tipe" disabled="disabled" value="<?php echo $data_akse['tipe_brg']; ?>"/></td>
    <td align="center"><input id="merk_akse" name="merk_akse" class="form-control" type="text"  placeholder="Merk" disabled="disabled" value="<?php echo $data_akse['merk_brg']; ?>" disabled="disabled"/></td>
