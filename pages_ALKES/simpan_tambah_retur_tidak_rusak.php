...
if (isset($_GET['simpan_barang'])==1) {
	$cek = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_kembali_tidak_rusak_hash where akun_id=".$_SESSION['id'].""));
	if ($cek!=0) {
	...
	$ins_brg_kembali = mysqli_query($koneksi, "insert into barang_kembali_tidak_rusak(no_retur,tgl_retur,barang_dikirim_id,no_po_id) values('".$_SESSION['no_retur']."','".$_SESSION['tgl_retur']."','".$_SESSION['no_po_id']."','".$s['no_po_jual']."')");
	
	//$total = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_kembali_tidak_rusak_hash"));
	
	//$simpan1=mysqli_query($koneksi, "insert into aksesoris_jual values('','".$_SESSION['tgl_jual']."','$total','$id_pembeli')");
	
	$d1=mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_kembali from barang_kembali_tidak_rusak"));
	$id_kembali=$d1['id_kembali'];
	//simpan barang pesan detail
	$q2=mysqli_query($koneksi, "select * from barang_kembali_tidak_rusak_hash where akun_id=".$_SESSION['id']."");
	while ($d2 = mysqli_fetch_array($q2)) {
		$simpan2=mysqli_query($koneksi, "insert into barang_kembali_tidak_rusak_detail(barang_kembali_tidak_rusak_id,barang_gudang_detail_id) values('$id_kembali','".$d2['barang_gudang_detail_id']."')");
		...
		$update=mysqli_query($koneksi, "update barang_gudang_detail set status_kirim=0,status_kerusakan=0,status_kembali_ke_gudang=0 where id=".$d2['barang_gudang_detail_id']."");
		$update2=mysqli_query($koneksi, "update barang_gudang,barang_gudang_detail set stok_total=stok_total+1 where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=".$d2['barang_gudang_detail_id']."");
		...
		if ($ins_brg_kembali and $simpan2) {
			mysqli_query($koneksi, "delete from barang_kembali_tidak_rusak_hash where akun_id=".$_SESSION['id']."");
			if ($_SESSION['user_admin_teknisi']) {
				echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=barang_kembali_tidak_rusak_teknisi'</script>";
				}
			else {
	echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=barang_kembali_tidak_rusak'</script>";
			}
		}
	}
	else {
		echo "<script>alert('Data Belum Diisi , Klik Tambah Terlebih Dahulu !');
		window.location='index.php?page=simpan_tambah_retur_tidak_rusak';
		</script>";
		}
}
...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	//$no=mysqli_num_rows(mysqli_query($koneksi, "select * from barang_kembali_tidak_rusak_hash"))+1;
	$simpan = mysqli_query($koneksi, "insert into barang_kembali_tidak_rusak_hash(akun_id,barang_gudang_detail_id) values('".$_SESSION['id']."','".$_POST['id_akse']."')");
	if ($simpan) {
		echo "<script>window.location='index.php?page=simpan_tambah_retur_tidak_rusak'</script>";
		}
	
}
	
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_kembali_tidak_rusak_hash where id=".$_GET['id_hapus']."");
	}
?>
...
<div class="content-wrapper">
  ...
  <thead>
    <tr>
      ...
      / Set</strong><td align="center" valign="bottom"><strong>Dinas/RS/Klinik/Dll      
      </strong>
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_kembali_tidak_rusak_hash.id as idd from barang_kembali_tidak_rusak_hash where akun_id=".$_SESSION['id']."");
  ...
  <tr>
    <td><?php echo $no; ?></td>
    <td><?php 
	$sel = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_gudang,barang_gudang_detail,barang_dikirim,barang_dikirim_detail,barang_dijual,pembeli,barang_kembali_tidak_rusak_hash where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dikirim_detail.barang_gudang_detail_id and barang_dijual.id=barang_dikirim.barang_dijual_id and barang_dikirim.id=barang_dikirim_detail.barang_dikirim_id and pembeli.id=barang_dijual.pembeli_id and barang_gudang_detail.id=barang_kembali_tidak_rusak_hash.barang_gudang_detail_id and barang_kembali_tidak_rusak_hash.id=".$data_akse['idd'].""));
	echo $sel['nama_brg']; ?>
    </td>
    <td align="center"><?php echo $sel['no_seri_brg']." / ".$sel['nama_set']; ?></td>
    <td align="center"><?php echo $sel['nama_pembeli']; ?></td>
    <td align="center"><a href="index.php?page=simpan_tambah_retur_tidak_rusak&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
...
<br />
<center><a href="index.php?page=simpan_tambah_retur_tidak_rusak&simpan_barang=1"><button name="simpan_barang" class="btn btn-success" type="submit"><span class="fa fa-plus"></span> Simpan</button></a>&nbsp;&nbsp;<a href="index.php?page=barang_kembali_tidak_rusak"><button name="batal" class="btn btn-success" type="submit"><span class="fa  fa-times-circle"></span> Batal</button></a></center>
</div>
              </div>
            </div>
          </div>
...
<div class="modal fade" id="modal-pilihnoseri">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <select name="id_akse" id="id_akse" class="form-control select2" autofocus="autofocus" style="width:100%" required onchange="changeValue(this.value)">
