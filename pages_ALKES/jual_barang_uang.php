...
if (isset($_GET['id_batal'])) {
	...
	if ($cek==0) {
	$del1=mysqli_query($koneksi, "delete from barang_dijual_qty,barang_dijual where barang_dijual.id=barang_dijual_qty.barang_dijual_id and no_po_jual='".$sel['no_po_jual']."'");
	if ($del1) {
    ...
    <section class="content">
      ...
      <section class="col-lg-12">
        <div class="box box-body">
          <?php if (isset($_SESSION['user_administrator']) or isset($_SESSION['adminkeuangan']) or isset($_SESSION['adminmanajerkeuangan']) or isset($_SESSION['adminpoluar'])) { ?>
...
?>
  <tr>
	...
	?></td>
    <td><?php $jm_deal=mysqli_num_rows(mysqli_query($koneksi, "select * from barang_dijual where no_po_jual='".$json[$i]['no_po_jual']."' and status_deal=1"));
    ...
    <?php if (!isset($_SESSION['adminmanajermarketing'])) { ?>
    <td align="center">
    <?php if (isset($_SESSION['user_administrator']) && isset($_SESSION['pass_administrator']) or isset($_SESSION['user_admin_keuangan']) && isset($_SESSION['pass_admin_keuangan']) or isset($_SESSION['adminpoluar'])) { ?>
  ...
  <div class="modal fade" id="modal-detailbarang<?php echo $json[$i]['idd']; ?>">
	...
	if ($jml_deal==0) {
	  ...
	  echo "<center><em>Riwayat Terakhir</em></center>";
		 ...
		 $q2=mysqli_query($koneksi, "select *,barang_dijual_qty.id as id_det_jual from barang_dijual_qty,barang_dijual,barang_gudang where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and barang_dijual.no_po_jual='".$json[$i]['no_po_jual']."' and status_deal=1");
	} 
	...
	if ($jml_deal==0) {
	  ...
	  echo "<center><em>Riwayat Terakhir</em></center>";
		 ...
		 $qq2=mysqli_query($koneksi, "select *,barang_dijual_qty.id as id_det_jual from barang_dijual_qty,barang_dijual,barang_gudang where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and barang_dijual.no_po_jual='".$json[$i]['no_po_jual']."' and status_deal=1");
	} 
...
$queryy = mysqli_query($koneksi, "select *,barang_dijual.id as idd from barang_dijual,barang_dijual_qty,barang_gudang,pembeli where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and status_awal=1 and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]' group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC");
	}
	elseif (isset($_GET['kunci']) and isset($_GET['pilihan'])) {
$queryy = mysqli_query($koneksi, "select *,barang_dijual.id as idd from barang_dijual,barang_dijual_qty,barang_gudang,pembeli where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and status_awal=1 and $_GET[pilihan] like '%$_GET[kunci]%' group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC");
	}
	else {
	$queryy = mysqli_query($koneksi, "SELECT * FROM barang_dijual where status_awal=1 group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC");
	}
