...
if (isset($_POST['pencarian'])) {
		...
		echo "<script>window.location='index.php?page=kirim_barang_demo&pilihan=$_POST[pilihan]&tgl1=$_POST[tgl1]&tgl2=$_POST[tgl2]&tampil=$_POST[tampil]'</script>";
		}
		elseif ($_POST['pilihan']=='tgl_sampai') {
		echo "<script>window.location='index.php?page=kirim_barang_demo&pilihan=$_POST[pilihan]&tgl1=$_POST[tgl1]&tgl2=$_POST[tgl2]&tampil=$_POST[tampil]'</script>";
		}
		 else {
		echo "<script>window.location='index.php?page=kirim_barang_demo&pilihan=$_POST[pilihan]&kunci=$_POST[kata_kunci]&tampil=$_POST[tampil]'</script>";
		}
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="box box-warning"><!-- /.chat -->
            <div class="box-footer">
              ...
              <a href="?page=kirim_barang_demo"><button class="btn btn-info pull pull-right"><span class="fa fa-refresh"></span> Reset</button></a>&nbsp;&nbsp;
              <?php } ?>
...
if (isset($_GET['pilihan']) and isset($_GET['kunci'])) {
	$file = file_get_contents("http://localhost/ALKES/json/kirim_barang_demo.php?pilihan=$_GET[pilihan]&kunci=".str_replace(" ","%20",$_GET['kunci'])."");
}
elseif (isset($_GET['pilihan']) and isset($_GET['tgl1']) and isset($_GET['tgl2'])) {
	$file = file_get_contents("http://localhost/ALKES/json/kirim_barang_demo.php?pilihan=$_GET[pilihan]&tgl1=".$_GET['tgl1']."&tgl2=".$_GET['tgl2']."");
}
else {
$file = file_get_contents("http://localhost/ALKES/json/kirim_barang_demo.php");
}
...
?>
  <tr>
    ...
    <a href="index.php?page=kirim_barang_demo&id_hapus=<?php echo $json[$i]['idd']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
    <?php } ?>
    <a href="index.php?page=ubah_barang_kirim_demo&id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a><br />
    
      <?php /* if (isset($_GET['id_krm'])) { ?>
      <a href="index.php?page=kirim_barang_demo&id=<?php echo $json[$i]['idd']; ?>&id_krm=<?php echo $_GET['id_krm']; ?>#openSampai"><span data-toggle="tooltip" title="Status : Sudah Sampai" class="fa fa-calendar-check-o"></span></a>&nbsp;&nbsp;
      <?php } else { ?>
      <a href="index.php?page=kirim_barang_demo&id=<?php echo $json[$i]['idd']; ?>#openSampai">
      <span data-toggle="tooltip" title="Status : Sudah Sampai" class="fa fa-calendar-check-o"></span>
      </a>&nbsp;&nbsp;
      <?php } */?>
      <!--<a href="index.php?page=kirim_barang_demo&id_b_s=<?php echo $json[$i]['idd']; ?>" onclick="return confirm('Yakin Akan Membatalkan Tanggal Sampai Barang !')">
      <span data-toggle="tooltip" title="Status : Belum Sampai" class="fa fa-calendar-times-o"></span>
      </a><br />-->
      <a href="cetak_surat_jalan_demo.php?id=<?php echo $json[$i]['idd']; ?>" target="_blank"><span data-toggle="tooltip" title="Cetak Surat Jalan" class="fa fa-print"></span></a><br />
      <?php	if ($json[$i]['tgl_sampai']!=0000-00-00) { ?>
      <a href="index.php?page=pilih_barang_demo_kembali&id=<?php echo $json[$i]['idd']; ?>&hapus_all=1"><small data-toggle="tooltip" title="" class="label bg-green"> Kembali</small></a>
      <?php } } ?>
    </td>
  </tr>
