...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	...
	$simpan = mysqli_query($koneksi, "insert into barang_pesan_detail_set values('','".$_GET['id']."','".$_POST['id_akse']."','".$_POST['qty']."','".$_POST['mata_uang']."','".$_POST['harga_perunit']."','".$_POST['diskon']."','".$_POST['total_harga']."','".$_POST['catatan_spek']."','')");
	echo "<script>window.location='index.php?page=tambah_po_alkes_set&id=$_GET[id]'</script>";
	}
...
";
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_detail_set.id as idd,barang_gudang_set.id as id_gudang from barang_pesan_detail_set,barang_gudang_set,mata_uang where barang_gudang_set.id=barang_pesan_detail_set.barang_gudang_set_id and mata_uang.id=barang_pesan_detail_set.mata_uang_id and barang_pesan_detail_set.barang_pesan_set_id=$_GET[id]");
  ...
  <tr>
    ...
    <td align="center"><a href="index.php?page=tambah_po_alkes_set&id_hapus=<?php echo $data_akse['idd']; ?>&id=<?php echo $_GET['id']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
    ...
    <td colspan="2" align="left">
      <?php
    $total = mysqli_fetch_array(mysqli_query($koneksi, "select *,sum(harga_total) as total from barang_pesan_detail_set where barang_pesan_set_id=$_GET[id]"));
    ...
    <td colspan="2" align="left" valign="top"><input name="cost_byair" id="cost_byair" class="form-control" type="text" required="required" value="<?php echo $data['cost_byair']; ?>" placeholder="" size="20" onkeyup="sum_total_keseluruhan();"/></td>
    <td align="center"></td>
    </tr>
    <tr>
...
<center><a href="index.php?page=tambah_po_alkes&simpan_barang=1&id=<?php echo $_GET['id']; ?>"><button name="simpan_barang" class="btn btn-success" type="submit"><span class="fa fa-check"></span> Simpan</button></a></center>
</form>
</div>
              </div>
            </div>
