...
if (isset($_POST['pencarian'])) {
		...
		echo "<script>window.location='index.php?page=kirim_barang_uang&tgl1=$_POST[tgl1]&tgl2=$_POST[tgl2]&tampil=$_POST[tampil]'</script>";
		} else {
		echo "<script>window.location='index.php?page=kirim_barang_uang&pilihan=$_POST[pilihan]&kunci=$_POST[kata_kunci]&tampil=$_POST[tampil]'</script>";
		}
	}
    ...
    <td>Jika <strong>Box</strong> Di <strong>Nama Alkes</strong> Berwarna <strong style="color:#F00">Merah</strong> , Itu menandakan<br /> 
      barang telah dikembalikan karena mengalami kerusakan</td>
             ...
             <?php if (isset($_GET['kunci']) or isset($_GET['tgl1'])) { ?>
              <a href="?page=kirim_barang"><button class="btn btn-info pull pull-right"><span class="fa fa-refresh"></span> Reset</button></a>&nbsp;&nbsp;
              <?php } ?>
...
?>
  <tr>
    ...
    <td><a href="#" data-toggle="modal" data-target="#modal-instalasi<?php echo $json[$i]['idd']; ?>"><small data-toggle="tooltip" title="Detail Barang" class="label bg-primary"><span class="fa fa-folder-open"></span></small></a></td>
    <td align="center">
    <a target="blank" href="cetak_faktur_penjualan.php?id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Cetak Faktur Penjualan" class="glyphicon glyphicon-print"></span></a>
    </td>
  </tr>
  <div class="modal fade" id="modal-detailbarang<?php echo $json[$i]['idd']; ?>">
      ...
      <?php 
	  $q33=mysqli_query($koneksi, "select tgl_i,tgl_f,nama_brg,tipe_brg,no_seri_brg from barang_gudang,barang_gudang_detail,barang_dikirim_detail,barang_teknisi_detail,alat_uji_detail where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dikirim_detail.barang_gudang_detail_id and barang_dikirim_detail.id=barang_teknisi_detail.barang_dikirim_detail_id and barang_teknisi_detail.id=alat_uji_detail.barang_teknisi_detail_id and barang_dikirim_id=".$json[$i]['idd']."");
	  $n=0;
	  while ($d1=mysqli_fetch_array($q33)) {
	  $n++;
	  ?>
      <?php if ($d1['status_batal']==1) { ?>
        <font class="pull pull-right" size="+1">Batal</font>
      ...
      <font class="pull pull-right">
      <?php echo "Instalasi : ".date("d/m/Y",strtotime($d1['tgl_i']))."<br>"; ?>
      <?php echo "Uji Fungsi : ".date("d/m/Y",strtotime($d1['tgl_f'])); ?>
      </font>
	  <?php echo $n.". ".$d1['nama_brg']." | "; ?>
      <?php echo " ".$d1['tipe_brg']." | "; ?>
      <?php echo " ".$d1['no_seri_brg']; ?>
