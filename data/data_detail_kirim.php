...
<div class="table-responsive no-padding">
    <table width="100%" id="example1" class="table table-bordered table-hover">
        ...
        while ($d_qty = mysqli_fetch_array($qty)) {
            $sel_satuan = mysqli_fetch_array(mysqli_query($koneksi, "select count(*) over () as jml, sum(jml_satuan*$d_qty[qty_jual]) as jml_rinci from barang_dijual_qty_detail where barang_dijual_qty_id=" . $d_qty['id_qty'] . ""));
            $sel_set = mysqli_fetch_array(mysqli_query($koneksi, "select sum(jml_satuan*$d_qty[qty_jual]) as jml_rinci from barang_dijual_qty_detail where barang_dijual_qty_id=" . $d_qty['id_qty'] . ""));
            ...
            $sel3 = mysqli_fetch_array(mysqli_query($koneksi, "select count(*) as jml from barang_dikirim_detail_hash where barang_dijual_qty_id=" . $d_qty['id_qty'] . " and akun_id = '$_SESSION[id]'"));
            ...
            <tr>
                ...
                <td>
                    ...
                    if ($d_qty['kategori_brg'] == 'Aksesoris') {
                        ...
                        echo $sel_satuan['jml_rinci'] + $d_qty['qty_jual'];
                    } else if ($d_qty['kategori_brg'] == 'Set') {
                        echo $sel_set['jml_rinci'];
                    }
                    ?>
                </td>
