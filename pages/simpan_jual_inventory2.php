...
if (isset($_POST['tambah_header'])) {
	$cek = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_dijual_inventory_qty_hash where akun_id=".$_SESSION['id'].""));
	if ($cek!=0) {
	...
	$total = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_dijual_inventory_qty_hash where akun_id=".$_SESSION['id'].""));
	
	$simpan1=mysqli_query($koneksi, "insert into barang_dijual_inventory values('','".$_SESSION['tgl_jual']."','".$_SESSION['no_faktur']."','$id_pembeli','".$_SESSION['marketing']."','".$_SESSION['subdis']."','".$_SESSION['diskon']."','".$_SESSION['ppn']."','".$_POST['nominal']."')");
	
	$d1=mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_jual from barang_dijual_inventory"));
	$id_jual=$d1['id_jual'];
	//simpan barang pesan detail
	$q2=mysqli_query($koneksi, "select * from barang_dijual_inventory_qty_hash where akun_id=".$_SESSION['id']."");
	while ($d2 = mysqli_fetch_array($q2)) {
		$simpan2=mysqli_query($koneksi, "insert into barang_dijual_inventory_qty values('','$id_jual','".$d2['barang_inventory_id']."','".$d2['harga_jual_saat_itu']."','".$d2['qty']."','0')");
		//$up=mysqli_query($koneksi, "update barang_gudang_detail set status_terjual=1 where id=".$d2['barang_gudang_detail_id']."");
		$up2=mysqli_query($koneksi, "update barang_inventory set stok_total=stok_total-$d2[qty] where id=".$d2['barang_inventory_id']."");
		}
		if ($simpan1 and $simpan2) {
			$Result = mysqli_query($koneksi, "insert into utang_piutang_inventory values('','Piutang','".$_SESSION['no_faktur']."','".$_POST['tgl_input']."','".$_POST['jatuh_tempo']."','".$_POST['nominal']."','".$_POST['klien']."','".$_POST['deskripsi']."','0')");
			mysqli_query($koneksi, "delete from barang_dijual_inventory_qty_hash where akun_id=".$_SESSION['id']."");
	echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=penjualan_inventory'</script>";
		}
	}
	else {
		echo "<script type='text/javascript'>
	alert('Data tidak boleh kosong , silakan tambah terlebih dahulu ! !');
	window.location='index.php?page=simpan_jual_inventory2'</script>";
		}
}


if (isset($_POST['simpan_tambah_aksesoris'])) {
	$stok = mysqli_fetch_array(mysqli_query($koneksi, "select stok_total from barang_inventory where id=".$_POST['id_akse'].""));
	$stok_po1 = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty_jual) as stok_po from barang_dijual_inventory_qty where barang_inventory_id=".$_POST['id_akse'].""));
	$stok_po2 = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_dikirim_inventory_detail,barang_inventory,barang_dijual_inventory_qty where barang_dijual_inventory_qty.id=barang_dikirim_inventory_detail.barang_dijual_inventory_qty_id and barang_inventory.id=barang_dijual_inventory_qty.barang_inventory_id and barang_dijual_inventory_qty.barang_inventory_id=".$_POST['id_akse'].""));
	$go=$stok['stok_total']-($stok_po1['stok_po']-$stok_po2);
	if ($go>=$_POST['qty']) {
		$sel_hrg_jual=mysqli_fetch_array(mysqli_query($koneksi, "select harga_satuan from barang_inventory where id=".$_POST['id_akse'].""));
	$simpan = mysqli_query($koneksi, "insert into barang_dijual_inventory_qty_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','".$sel_hrg_jual['harga_satuan']."','".$_POST['qty']."')");
	if ($simpan) {
		echo "<script>window.location='index.php?page=simpan_jual_inventory2'</script>";
		}
	} else {
...
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_dijual_inventory_qty_hash where id=".$_GET['id_hapus']."");
	}
?>
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
      Jual Barang Inventory</h1><ol class="breadcrumb">
       <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active"> Jual Barang Inventory</li>
      </ol>
    </section>
  ...
  <thead>
    <tr>
      ...
      <td align="center" valign="bottom"><strong>Stok_Gudang - Stok_PO</strong>      
      ...
      <td valign="bottom"><strong>Total</strong>      
  ...
  <tr>
    ...
    <select name="id_akse" id="id_akse" class="form-control select2" autofocus="autofocus" required onchange="changeValue(this.value)">
        ...
        <?php 
		$q = mysqli_query($koneksi, "select * from barang_inventory where stok_total!=0 order by nama_brg ASC");
...
";
	...
	$stok_po2 = mysqli_num_rows(mysqli_query($koneksi, ""));
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_dijual_inventory_qty_hash.id as idd from barang_dijual_inventory_qty_hash,barang_inventory where barang_inventory.id=barang_dijual_inventory_qty_hash.barang_inventory_id and barang_dijual_inventory_qty_hash.akun_id=".$_SESSION['id']."");
  ...
  <tr>
    ...
    <td align="center">&nbsp;</td>
    <td align="center"><?php echo "Rp".number_format($data_akse['harga_satuan'],2,',','.'); ?></td>
    ...
    <td><?php echo "Rp".number_format($data_akse['harga_satuan']*$data_akse['qty'],2,',','.'); ?></td>
    <td align="center"><a href="index.php?page=simpan_jual_inventory&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
    ...
    <tr>
      ...
      <td><?php
      $total1 = mysqli_fetch_array(mysqli_query($koneksi, "select sum(qty*harga_satuan) as total1 from barang_inventory,barang_dijual_inventory_qty_hash where barang_inventory.id=barang_dijual_inventory_qty_hash.barang_inventory_id and akun_id=".$_SESSION['id'].""));
    ...
    <tr>
      ...
      <td>
      <?php 
	  $ppn = $total1['total1']*$_SESSION['ppn']/100;
    ...
    <tr>
      ...
      <td><?php 
	  $diskon = $total1['total1']*$_SESSION['diskon']/100;
...
function yesnoCheck() {
    if (document.getElementById('yesCheck').checked) {
     ...
     <label>No PO</label>
              ...
              <label>Klien</label>
              <input name="klien" class="form-control" type="text" placeholder="" value="<?php $pem = mysqli_fetch_array(mysqli_query($koneksi, "select * from pembeli where id=".$_SESSION['pembeli']."")); echo $pem['nama_pembeli'];  ?>" required="required"><br />
