...
if (isset($_POST['tambah_laporan'])) {
	$dat=mysqli_num_rows(mysqli_query($koneksi, "select * from akun_cs where username='".$_POST['user']."' and password='".md5($_POST['pass'])."'"));
	if ($dat==0) {
	$Result = mysqli_query($koneksi, "insert into akun_cs values('','".$_POST['nama']."','".$_POST['user']."','".md5($_POST['pass'])."')");
		if ($Result) {
			echo "<script type='text/javascript'>
			alert('Data Berhasil Di Tambah !');
			window.location='index.php?page=akun_cs'
			</script>";
			}
	} else {
		echo "<script type='text/javascript'>
