...
if (isset($_POST['simpan_tambah_aksesoris'])) {
	...
	if ($simpan) {
		echo "<script>
		window.location='index.php?page=pilih_peminjaman_barang'</script>";
		}
	} 
	else {
		echo "<script>
...
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_pinjam_detail_hash where id=".$_GET['id_hapus']."");
	}
?>
...
<div class="content-wrapper">
  ...
  <?php }} ?>
              ...
              <center>
               ...
               <button name="tambah_laporan" class="btn btn-success pull pull-left pull-top" type="button" data-toggle="modal" data-target="#modal-pilihnoseri"><span class="fa fa-plus"></span> Pilih Barang </button>
               <br /><br /><br />
                <div class="table-responsive">
  ...
  <tr>
    ...
    <td><a href="index.php?page=pilih_peminjaman_barang&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php } ?>
</table>
                </div>
...
-->
  ...
  <div class="modal fade" id="modal-pilihnoseri">
      ...
      <select name="id_akse" id="id_akse" class="form-control" autofocus="autofocus" required onchange="changeValue(this.value)">
        ...
        <?php 
		$q = mysqli_query($koneksi, "select *,barang_gudang.id as idd from barang_dikirim,barang_dikirim_detail,barang_dijual,pembeli,barang_gudang,barang_gudang_detail where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dikirim_detail.barang_gudang_detail_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.id=barang_dikirim.barang_dijual_id and barang_dikirim.id=barang_dikirim_detail.barang_dikirim_id and barang_dikirim_id=".$_SESSION['barang_dikirim_id']." group by barang_gudang.id order by nama_brg ASC");
...
";
    ...
    <?php
	$q_seri = mysqli_query($koneksi, "select *,barang_gudang_detail.id as idd from barang_dikirim,barang_dikirim_detail,barang_dijual,pembeli,barang_gudang,barang_gudang_detail where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dikirim_detail.barang_gudang_detail_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.id=barang_dikirim.barang_dijual_id and barang_dikirim.id=barang_dikirim_detail.barang_dikirim_id and barang_dikirim_id=".$_SESSION['barang_dikirim_id']." order by no_seri_brg");
