...
if (isset($_POST['tambah_header'])) {
	...
	if ($cek_saldo['saldo']<$_POST['nominal']) {
		...
		window.location='index.php?page=bayar_utang_set&id=$_GET[id]'
		</script>";
		}
	else {
	$Result = mysqli_query($koneksi, "insert into utang_piutang_set_bayar values('','Hutang','$_GET[id]','".$_POST['tgl_input']."','".$_POST['nominal']."','".$_POST['deskripsi']."','".$_POST['akun']."')");
	if ($Result) {
		...
		if ($sel['jumlah']>=$data['nominal']) {
			mysqli_query($koneksi, "update utang_piutang_set,barang_pesan_set set utang_piutang_set.status_lunas=1,barang_pesan_set.status_lunas=1 where barang_pesan_set.no_po_pesan=utang_piutang_set.no_faktur_no_po_set and utang_piutang_set.id=$_GET[id]");
			}
			echo "<script type='text/javascript'>
...
if (isset($_GET['id_hapus'])) {
	...
	$up = mysqli_query($koneksi, "update utang_piutang_set,barang_pesan_set set utang_piutang_set.status_lunas=0,barang_pesan_set.status_lunas=0 where barang_pesan_set.no_po_pesan=utang_piutang_set.no_faktur_no_po_set and utang_piutang_set.id=$_GET[id]");
		}
		}
	}
	echo "<script>window.location='index.php?page=bayar_utang_set&id=$_GET[id]'</script>";
...
<?php if ($data['status_lunas']==0){$b="#FF0000";} else {$b="#00CC66";} ?>
  <tr>
    ...
    <td><?php echo $data['no_faktur_no_po_set']; ?></td>
  ...
  <?php 
  $q2=mysqli_query($koneksi, "select *,utang_piutang_set_bayar.id as idd from utang_piutang_set_bayar,buku_kas where buku_kas.id=utang_piutang_set_bayar.buku_kas_id and u_p='Hutang' and utang_piutang_set_id=$_GET[id]");
  ...
  <tr>
    ...
    <td><?php echo "Rp ".number_format($d['nominal'],2,',','.'); ?>
      ...
      <td><a href="index.php?page=bayar_utang_set&id_hapus=<?php echo $d['idd']; ?>&id=<?php echo $_GET['id']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Riwayat Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
        <a href="index.php?page=bayar_utang_set&id_ubah=<?php echo $d['idd']; ?>&id=<?php echo $_GET['id']; ?>#openUbah"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a></td>
    </tr>
 <?php } ?>
</table>  
			  <script type="text/javascript">
function yesnoCheck() {
  ...
  <?php
  $da = mysqli_fetch_array(mysqli_query($koneksi, "select *,buku_kas.id as idd from utang_piutang_set_bayar,buku_kas where buku_kas.id=utang_piutang_set_bayar.buku_kas_id and utang_piutang_set_bayar.id=$_GET[id_ubah]"));
