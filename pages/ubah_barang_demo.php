...
if (isset($_GET['id_hapus'])) {
  ...
  if ($del1) {
    ...
    Swal.fire({
      ...
      window.location = '?page=$_GET[page]&id_ubah=$_GET[id_ubah]'
    })
    </script>";
  } else {
    echo "<script>
...
";
                                while ($d2 = mysqli_fetch_array($qq)) { ?>
                                  <option value="<?php echo $d2['id']; ?>" <?php if ($d2['id'] == $ss['barang_gudang_id']) {
                                                                              echo "selected";
                                                                            } ?>><?php echo $d2['nama_brg']; ?></option>
                                <?php
  ...
  function hapus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = '?page=' + getVars("page").replace('#', '') + '&id_hapus=' + id + "&id_ubah=" + getVars("id_ubah").replace('#', '');
      }
    })
  }
</script>