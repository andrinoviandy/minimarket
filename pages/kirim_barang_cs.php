...
if (isset($_POST['pencarian'])) {
		...
		echo "<script>window.location='index.php?page=kirim_barang_cs&tgl1=$_POST[tgl1]&tgl2=$_POST[tgl2]&tampil=$_POST[tampil]'</script>";
		} else {
		echo "<script>window.location='index.php?page=kirim_barang_cs&pilihan=$_POST[pilihan]&kunci=$_POST[kata_kunci]&tampil=$_POST[tampil]'</script>";
		}
	}
...
if (isset($_POST['sampai_barang'])) {
	  ...
	  $que = mysqli_query($koneksi, "update barang_dikirim set estimasi_barang_sampai='".$_POST['estimasi_barang_sampai']."', tgl_sampai='".$_POST['tgl_sampai']."' where id=".$_POST['id_status']."");
	  if ($que) {
		  //mysqli_query($koneksi, "insert into uji_f_i values('','".$_GET['id']."','0','0','')");
		  echo "<script type='text/javascript'>
		  window.location='index.php?page=kirim_barang_cs'
		  </script>";
		  }
	  	} else {
...
if (isset($_GET['id_b_s'])) {
	...
	if ($q) {
		echo "<script>window.location='index.php?page=kirim_barang_cs'</script>";
		}
	}
...
barang telah dikembalikan karena mengalami kerusakan</td>
             ...
             <?php if (isset($_GET['kunci']) or isset($_GET['tgl1'])) { ?>
              <a href="?page=kirim_barang_cs"><button class="btn btn-info pull pull-right"><span class="fa fa-refresh"></span> Reset</button></a>&nbsp;&nbsp;
              <?php } ?>
...
?>
  <tr>
    ...
    <td>
    <?php 
	$data4=mysqli_fetch_array(mysqli_query($koneksi, "select nama_pemakai,kontak1_pemakai,kontak2_pemakai from pemakai,barang_dijual,barang_dikirim where pemakai.id=barang_dijual.pemakai_id and barang_dijual.id=barang_dikirim.barang_dijual_id and barang_dikirim.id=".$json[$i]['idd'].""));
	echo $data4['nama_pemakai']; ?>
    </td>
    <td><?php echo $data4['kontak1_pemakai']." / ".$data4['kontak2_pemakai'] ?></td>
    ...
    <a href="index.php?page=ubah_barang_kirim&id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a><!--<a target="blank" href="cetak_surat_perintah_instalasi.php?id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Cetak Surat Perintah Instalasi" class="glyphicon glyphicon-print"></span></a><a href="cetak_surat_perintah_instalasi.php?id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Buat Surat Perintah Instalasi" class="fa fa-file-pdf-o"></span></a>--><br />
      ...
      <a href="#" data-toggle="modal" data-target="#modal-status<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Status : Sudah Sampai" class="fa fa-calendar-check-o"></span></a>&nbsp;&nbsp;
      <?php } else { ?>
      ...
      <small data-toggle="tooltip" title="Riwayat Panggilan" class="label bg-green">Riwayat Panggilan
      <?php $jmll = mysqli_num_rows(mysqli_query($koneksi, "select * from riwayat_panggilan where barang_dikirim_id=".$json[$i]['idd'].""));
	  echo " (".$jmll.")"; ?>
      </small>
	  </a>
      <?php if (isset($_GET['id_krm'])) { ?>
