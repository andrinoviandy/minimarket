...
if (isset($_POST['kirim_barang'])) {
	  $input = mysqli_query($koneksi, "update barang_kembali_teknisi set status_progress_k_p=".$_POST['status']." where id=".$_GET['id_ubah']."");
		if ($input) {
		  		echo "<script>
			window.location='index.php?page=tambah_progress_barang_kembali&id_gudang_detail	=$_GET[id_gudang_detail]&id_ubah=$_GET[id_ubah]&id_gudang=$_GET[id_gudang]'
				</script>";
		  		}
	  }
 
if (isset($_GET['id_hapus'])) {
	$sel = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_kembali_teknisi_progress where id=".$_GET['id_hapus'].""));
	unlink("gambar_progress_barang_kembali/".$sel['lampiran_k_p']);
	$del = mysqli_query($koneksi, "delete from barang_kembali_teknisi_progress where id=".$_GET['id_hapus']."");
	if ($del) {
		echo "<script>
		window.location='index.php?page=tambah_progress_barang_kembali&id_gudang_detail=$_GET[id_gudang_detail]&id_ubah=$_GET[id_ubah]&id_gudang=$_GET[id_gudang]';
		</script>";
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            ...
            <div class="box-body">
              ...
              -->
              <a href="index.php?page=progress_barang_kembali_detail&id_gudang=<?php echo $_GET['id_gudang']; ?>"><button class="btn btn-success">Kembali Ke Halaman Sebelumnya</button></a>
...
<br />
<?php 
	$da = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_kembali_teknisi where id=".$_GET['id_ubah'].""));
	if ($da['status_progress_k_p']==0) { ?>
<a href="index.php?page=tambah_progress_barang_kembali2&id_gudang_detail=<?php echo $_GET['id_gudang_detail']; ?>&id_ubah=<?php echo $_GET['id_ubah']; ?>&id_gudang=<?php echo $_GET['id_gudang']; ?>"><button class="btn btn-success"><span class="fa fa-plus"></span> &nbsp;Tambah</button></a>
<?php } ?>
<a target="_blank" href="cetak_progress_barang_kembali.php?id_gudang_detail=<?php echo $_GET['id_gudang_detail']; ?>&id_ubah=<?php echo $_GET['id_ubah']; ?>&id_gudang=<?php echo $_GET['id_gudang']; ?>"><button class="btn btn-info"><span class="fa fa-print"></span> &nbsp;Cetak Progress</button></a>
...
<table width="100%" id="example1" class="table table-bordered table-hover">
  ...
  <?php
	  ...
	  $query = mysqli_query($koneksi, "select * from barang_kembali_teknisi_progress where barang_kembali_teknisi_id=".$_GET['id_ubah']."");
  ...
  ?>
  <tr>
    <td><?php echo date("d-m-Y",strtotime($data['tgl_k_p'])); ?></td>
    <td><?php echo $data['kerusakan_k_p']; ?></td>
    <td><?php echo $data['perbaikan_k_p']; ?></td>
    <td><?php if ($data['lampiran_k_p']!="") { ?>
    <a href="gambar_progress_barang_kembali/<?php echo $data['lampiran_k_p']; ?>" target="_blank"><img src="gambar_progress_barang_kembali/<?php echo $data['lampiran_k_p']; ?>" width="50px" /></a>
    <?php } ?></td>
    <td>
    <?php 
	//$d = mysqli_fetch_array(mysqli_query($koneksi, "select * from tb_maintenance_detail.id="));
	if ($da['status_progress_k_p']==0) { ?>
    <a href="index.php?page=tambah_progress_barang_kembali&id_gudang_detail=<?php echo $_GET['id_gudang_detail']; ?>&id_ubah=<?php echo $_GET['id_ubah']; ?>&id_gudang=<?php echo $_GET['id_gudang']; ?>&id_hapus=<?php echo $data['id']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>
    <?php } ?>
    </td>
  ...
  <div class="modal fade" id="modal-status">
     ...
     <?php if ($da['status_progress_k_p']==0) { ?>
     	<option value="0">Belum Selesai</option>
        <option value="1">Sudah Selesai</option>
        <?php } else if ($da['status_progress_k_p']==1){ ?>
        <option value="1">Sudah Selesai</option>
        <option value="0">Belum Selesai</option>
        <?php } ?>
     </select>
