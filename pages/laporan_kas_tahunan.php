...
<div class="content-wrapper">
            ...
            <div class="box-footer">
              ...
              <tr>
                <td><select class="form-control" name="tahun" required>
                    ...
                    <?php 
					$tahun=date('Y');
					for ($j=$tahun-5; $j<$tahun; $j++) {
					?>
                  <option value="<?php echo $j; ?>"><?php echo $j; ?></option>  
                    <?php } ?>
                  <?php
                  for ($k=$tahun; $k<$tahun+5; $k++) {
					?>
              ...
              <table width="100%" class="table no-border">
                ...
                <tr style="background-color:#0CC">
				  ...
				  if (isset($_POST['lihat'])) {
					  if ($_POST['akun']=='all') {
						$piutang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".$_POST['tahun']." and u_p='Piutang'"));  
						  }
					  else {
					  $piutang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".$_POST['tahun']." and u_p='Piutang' and buku_kas_id=".$_POST['akun'].""));
					  }
					  }
				  else {
					$piutang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".date('Y')." and u_p='Piutang'"));
				  }
                ...
                <tr style="background-color:#F30">
				  ...
				  if (isset($_POST['lihat'])) {
					  if ($_POST['akun']=='all') {
							$utang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".$_POST['tahun']." and u_p='Hutang'"));	  
						  }
					  else {
				  $utang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".$_POST['tahun']." and u_p='Hutang' and buku_kas_id=".$_POST['akun'].""));
					  }
				  }
				  else {
					$utang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where YEAR(tgl_bayar)=".date('Y')." and u_p='Hutang'"));
				  }
  ...
  if (isset($_POST['lihat'])) {
	  if ($_POST['akun']=='all') {
		$query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and buku_kas.id=utang_piutang_bayar.buku_kas_id and YEAR(tgl_bayar)=".$_POST['tahun']." and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");  
		  }
	  else {
	  $query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and buku_kas.id=utang_piutang_bayar.buku_kas_id and YEAR(tgl_bayar)=".$_POST['tahun']." and buku_kas.id=".$_POST['akun']." and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");
	  }
	  }
  else {
	  $query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and YEAR(tgl_bayar)=".date('Y')." and buku_kas.id=utang_piutang_bayar.buku_kas_id and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");
  }
  $no=0;
