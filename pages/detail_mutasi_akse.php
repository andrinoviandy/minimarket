...
if (isset($_GET['batal_mutasi'])) {
	$sel1 = mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_pesan_akse_detail.id as idd from barang_pesan_akse,barang_pesan_akse_detail,aksesoris where aksesoris.id=barang_pesan_akse_detail.aksesoris_id and barang_pesan_akse.id=barang_pesan_akse_detail.barang_pesan_akse_id and barang_pesan_akse_detail.id=".$_GET['id_detail'].""));
	$sel2 = mysqli_fetch_array(mysqli_query($koneksi, "select * from aksesoris_po where no_po_akse='".$sel1['no_po_pesan']."' and aksesoris_id=".$_GET['id_gudang']." and stok=".$_GET['stok'].""));
	$sel3 = mysqli_num_rows(mysqli_query($koneksi, "select * from aksesoris_detail where aksesoris_po_id=".$sel2['id']." and status_kirim_akse=1"));
	
	if ($sel3!=0) {
	echo "<script type='text/javascript'>
		alert('Pembatalan Mutasi Barang GAGAL ! Barang Dengan No Po Ini Sudah Ada Yang Terjual !');
		window.location='index.php?page=detail_mutasi_akse&id=$_GET[id]&id_gudang=$_GET[id_gudang]&id_detail=$_GET[id_detail]';
		</script>";
	}
	else {
		$sel4 = mysqli_num_rows(mysqli_query($koneksi, "select * from aksesoris_detail where aksesoris_po_id=".$sel2['id'].""));
	$del1 = mysqli_query($koneksi, "delete from aksesoris_detail where aksesoris_po_id=".$sel2['id']."");
	$del2 = mysqli_query($koneksi, "delete from aksesoris_po where id=".$sel2['id']."");
	$del3 = mysqli_query($koneksi, "update aksesoris set stok_total_akse=stok_total_akse-$sel4 where id=".$_GET['id_gudang']."");
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       Riwayat Mutasi Aksesoris</h1><ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active"><a href="index.php?page=barang_masuk">Pemesanan</a></li>
        <li class="active">Detail Mutasi Alkes</li></ol></section>
  ...
  <?php
  $d0 = mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_pesan_akse_detail.id as idd from aksesoris,barang_pesan_akse_detail,barang_pesan_akse,principle where aksesoris.id=barang_pesan_akse_detail.aksesoris_id and barang_pesan_akse.id=barang_pesan_akse_detail.barang_pesan_akse_id and principle.id=barang_pesan_akse.principle_id and barang_pesan_akse.id=".$_GET['id'].""));
  ?>
  <tr>
    ...
    <td><?php echo $d0['nama_principle']; ?></td>
            ...
            <table  width="100%" id="example1" class="table table-bordered table-hover">
              <thead>
  <tr>
    <th><strong>Nama Aksesoris</strong></th>
    <th>Merk</th>
  ...
  <?php
  $q = mysqli_query($koneksi, "select *,barang_pesan_akse_detail.id as idd from aksesoris,barang_pesan_akse_detail,barang_pesan_akse where aksesoris.id=barang_pesan_akse_detail.aksesoris_id and barang_pesan_akse.id=barang_pesan_akse_detail.barang_pesan_akse_id and barang_pesan_akse.id=".$_GET['id']."");
  $no=0;
  while ($d = mysqli_fetch_array($q)) {
  $no++;
  ?>
  <tr>
    <td><?php echo $d['nama_akse']; ?></td>
    <td><?php echo $d['merk_akse']; ?></td>
    <td><?php echo $d['tipe_akse']; ?></td>
    <td><?php 
	$stok_sudah_mutasi = mysqli_fetch_array(mysqli_query($koneksi,"select sum(stok_masuk_akse) as stok_sudah from aksesoris_po where no_po_akse='".$d['no_po_pesan']."' and aksesoris_id=".$d['aksesoris_id'].""));
		  echo $d['qty']-$stok_sudah_mutasi['stok_sudah']; ?></td>
    <td><?php echo $stok_sudah_mutasi['stok_sudah']; ?></td>
    <td><span class="fa fa-euro"></span> <?php echo number_format($d['harga_perunit'],0,',',',').".00"; ?></td>
    <td align="center"><?php echo $d['diskon']." %"; ?></td>
    <td><span class="fa fa-euro"></span> <?php echo number_format($d['harga_total'],0,',',',').".00"; ?></td>
    </tr>
  <?php } ?>
</table>
            </div>
  ...
  <?php
  $q22 = mysqli_query($koneksi, "select *,aksesoris_po.id as idd from aksesoris_po,barang_pesan_akse where barang_pesan_akse.no_po_pesan=aksesoris_po.no_po_akse and barang_pesan_akse.id=".$_GET['id']." and aksesoris_id=".$_GET['id_gudang']."");
  $no=0;
  while ($d = mysqli_fetch_array($q22)) {
  $no++;
  ?>
  <tr>
    <td><?php echo date("d/m/Y",strtotime($d['tgl_masuk_akse'])); ?></td>
    <td><?php echo $d['stok_masuk_akse']; ?></td>
    <td align="center">
    <a href="index.php?page=detail_mutasi_akse&batal_mutasi=1&id=<?php echo $_GET['id']; ?>&id_gudang=<?php echo $_GET['id_gudang']; ?>&id_detail=<?php echo $_GET['id_detail']; ?>&stok=<?php echo $d['stok_masuk_akse']; ?>" onclick="return confirm('Anda Yakin Ingin Membatalkan Mutasi Stok Ini ?')"><small data-toggle="tooltip" title="Batalkan Mutasi" class="label bg-red"><span class="fa fa-close"></span> Batalkan Mutasi</small></a>
    </td>
  </tr>
  <?php } ?>
</table>
            ...
            <br />
            <center><a href="?page=mutasi_akse&id=<?php echo $_GET['id'] ?>"><button class="btn btn-success">Kembali</button></a></center>
              </div>
            </div>
          </div>
          <!-- /.box (chat box) -->
...
";
		while ($d = mysqli_fetch_array($q)) { ?>
        <option value="<?php echo $d['id']; ?>" <?php if ($d['id']==$data['id_principle']) { echo "selected";} ?>><?php echo $d['nama_principle']; ?></option>
        <?php 
