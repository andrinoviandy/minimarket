...
if (isset($_POST['pencarian'])) {
		...
		echo "<script>window.location='index.php?page=piutang_aksesoris&tgl1=$_POST[tgl1]&tgl2=$_POST[tgl2]'</script>";
		} else {
		echo "<script>window.location='index.php?page=piutang_aksesoris&pilihan=$_POST[pilihan]&kunci=$_POST[kata_kunci]'</script>";
		}
	}
  ...
  <thead>
    <tr>
      ...
      <td width="9%" align="center"><strong>Status</strong>
        ...
        <th width="2%" valign="top">ID</th>
        <th width="17%" valign="top"><strong>Tanggal </strong></th>
        <th width="15%" valign="top">No PO</th>
        <th width="15%" valign="top">Klien</th>
      ...
      <th width="15%" align="center" valign="top"><strong>Aksi</strong></th>
          </tr>
  </thead>
  <?php
...
<?php if ($json[$i]['status_lunas']==0){$b="#FF0000";} else {$b="#00CC66";} ?>
  <tr>
	...
	?></td>
    <td style="background-color:<?php echo $b; ?>;" align="center"><?php if ($json[$i]['status_lunas']==0){echo "Belum Lunas";} else {echo "Sudah Lunas";} ?></td>
    ...
    <td>
      ...
      <a href="index.php?page=piutang_aksesoris&id_hapus=<?php echo $json[$i]['idd']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
      <a href="index.php?page=ubah_piutang_aksesoris&id=<?php echo $json[$i]['idd']; ?>"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a><br />
      <?php }*/ ?>
	  <?php if ($json[$i]['status_lunas']==0) { ?>
      <a href="index.php?page=bayar_piutang_aksesoris&id=<?php echo $json[$i]['idd']; ?>"><small data-toggle="tooltip" title="Bayar" class="label bg-green"><span class="fa fa-plus"></span> Pembayaran</small></a>
      <?php } else { ?>
      <a href="index.php?page=bayar_piutang_aksesoris&id=<?php echo $json[$i]['idd']; ?>"><small data-toggle="tooltip" title="Batalkan" class="label bg-yellow">Riwayat Pembayaran</small></a>
      <?php } ?>
...
elseif (isset($_GET['kunci']) and isset($_GET['pilihan'])) {
	...
	$queryy = mysqli_query($koneksi, "select *,utang_piutang_aksesoris.id as idd from utang_piutang_aksesoris where u_p='Piutang' and nominal!=0 order by tgl_input DESC, utang_piutang_aksesoris.id DESC");
	}
