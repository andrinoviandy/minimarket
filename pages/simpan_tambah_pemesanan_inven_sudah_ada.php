...
if (isset($_POST['tambah_header'])) {
  //simpan barang pesan
  $ce = mysqli_num_rows(mysqli_query($koneksi, "select * from barang_pesan_inventory_detail_hash where akun_id=" . $_SESSION['id'] . ""));
  if ($ce != 0) {
    $max_id_princ = $_SESSION['id_princ'];
    $almat_peng = str_replace("\n", "<br>", $_SESSION['alamat_pengiriman']);
    $simpan1 = mysqli_query($koneksi, "insert into barang_pesan_inventory values('','" . $_SESSION['tgl_po'] . "','" . $_SESSION['no_po'] . "','Dalam Negeri','$max_id_princ','" . $_SESSION['ppn'] . "','" . $_SESSION['cara_pembayaran'] . "','" . $_SESSION['mata_uang'] . "','" . $almat_peng . "','" . $_SESSION['jalur_pengiriman'] . "','" . $_SESSION['estimasi_pengiriman'] . "','" . $_SESSION['catatan'] . "','0','','" . $_POST['total_price'] . "','" . $_POST['total_price_ppn'] . "','" . $_POST['cost_byair'] . "','" . $_POST['cost_cf'] . "','','','')");
    $d1 = mysqli_fetch_array(mysqli_query($koneksi, "select max(id) as id_pesan from barang_pesan_inventory"));
    $id_pesan = $d1['id_pesan'];
    //simpan barang pesan detail
    $q2 = mysqli_query($koneksi, "select * from barang_pesan_inventory_detail_hash where akun_id=" . $_SESSION['id'] . "");
    while ($d2 = mysqli_fetch_array($q2)) {

      $simpan2 = mysqli_query($koneksi, "insert into barang_pesan_inventory_detail values('','$id_pesan','" . $d2['barang_inventory_id'] . "','" . $d2['qty'] . "','" . $d2['mata_uang_id'] . "','" . $d2['harga_perunit'] . "','" . $d2['diskon'] . "','" . $d2['harga_total'] . "','" . $d2['catatan'] . "','0')");
    }

    if ($simpan1 and $simpan2) {
      $Result = mysqli_query($koneksi, "insert into utang_piutang_inventory values('','Hutang','" . $_SESSION['no_po'] . "','" . $_POST['tgl_input'] . "','" . $_POST['jatuh_tempo'] . "','" . $_POST['nominal'] . "','" . $_POST['klien'] . "','" . $_POST['deskripsi'] . "','0')");
      mysqli_query($koneksi, "delete from barang_pesan_inventory_detail_hash where akun_id=" . $_SESSION['id'] . "");
      echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=pembelian_inventory'</script>";
    }
  } else {
    echo "<script type='text/javascript'>
	alert('Silkan Tambah Data Terlebih Dahulu !');
	window.location='index.php?page=simpan_tambah_pemesanan_inven_sudah_ada'</script>";
  }
}

if (isset($_POST['simpan_tambah_aksesoris'])) {
  $simpan = mysqli_query($koneksi, "insert into barang_pesan_inventory_detail_hash values('','" . $_SESSION['id'] . "','" . $_POST['id_akse'] . "','" . $_POST['qty'] . "','" . $_SESSION['mata_uang'] . "','" . $_POST['harga_perunit'] . "','" . $_POST['diskon'] . "','" . $_POST['total_harga'] . "','" . $_POST['catatan_spek'] . "')");
  echo "<script>window.location='index.php?page=simpan_tambah_pemesanan_inven_sudah_ada'</script>";
}

if (isset($_GET['id_hapus'])) {
  $del = mysqli_query($koneksi, "delete from barang_pesan_inventory_detail_hash where id=" . $_GET['id_hapus'] . "");
}
?>
                ...
                <table width="100%" id="" class="table table-bordered table-hover">
                  <thead>
                    <tr>
                      ...
                      <td align="center" valign="bottom"><strong>Catatan</strong></td>
                  ...
                  <tr>
                    ...
                    <form method="post" enctype="multipart/form-data">
                      <td class="">
                        ...
                        <select name="id_akse" id="id_akse" class="form-control select2 min-vw-100" autofocus="autofocus" required onchange="changeValue(this.value)">
...
";
                      ...
                      <td align="center">
                        ...
                        ?>
                        <!--<a href="index.php?page=simpan_tambah_pemesanan_alkes#openUang"><small data-toggle="tooltip" title="Tambah Mata Uang" class="label bg-green pull pull-right">+</small></a>-->
                      </td>
                      <td align="center"><input id="harga_perunit" name="harga_perunit" class="form-control" type="text" placeholder="" required="required" size="10" onkeyup="sum();" /></td>
                      <td align="center"><input id="diskon" name="diskon" onkeyup="sum();" class="form-control" type="text" placeholder="" size="5" required="required" /></td>
                      <td align="center">

                        <input id="total_harga" name="total_harga" class="form-control" type="text" placeholder="" readonly="readonly" size="10" />
                      </td>
                  ...
                  $no = 0;
                  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_inventory_detail_hash.id as idd from barang_pesan_inventory_detail_hash,mata_uang,barang_inventory where mata_uang.id=barang_pesan_inventory_detail_hash.mata_uang_id and barang_inventory.id=barang_pesan_inventory_detail_hash.barang_inventory_id and akun_id=" . $_SESSION['id'] . "");
                  ...
                  ?>
                      <tr>
                        ...
                        <td align="center"><a href="index.php?page=simpan_tambah_pemesanan_inven_sudah_ada&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
                      </tr>
                  <?php }
                  } else {
                  ...
                  <form method="post">
                    <tr>
                      ...
                      <td align="center">
                        <?php
                        $total = mysqli_fetch_array(mysqli_query($koneksi, "select sum(harga_total) as total from barang_pesan_inventory_detail_hash"));
                    ...
                    <tr>
                      ...
                      <td align="center" valign="top"><input name="cost_byair" id="cost_byair" class="form-control" type="text" onkeyup="sum_total_keseluruhan();" placeholder="" size="10" /></td>
                      <td align="center"></td>
                      <td align="center"></td>
                    </tr>
                    <tr>
                      <td colspan="6" align="right" valign="bottom"><strong>Total Keseluruhan =</strong></td>
...
<div id="open_utang" class="modalDialog">
  <div>
    ...
    <label>Nominal</label>
    <input id="nominal" name="nominal" class="form-control" type="text" placeholder="Dalam Rupiah" value=""><br />
    <label>Klien</label>
