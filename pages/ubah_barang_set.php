...
<script>
  function hapus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.post("data/hapus-barang-set.php", {
            id_hapus: id
          },
          function(data) {
            if (data == 'S') {
  ...
  function getData() {
    $('#detail-barang-set').html('<center><div class="overlay" style="margin-top:200px"><i class="fa fa-refresh fa-spin"></i></div></center>');
    $.get("data/detail-barang-set.php", {
        id: '<?php echo $_GET['id'] ?>'
      },
      function(data) {
        $('#detail-barang-set').html(data);
      }
    );
  }
