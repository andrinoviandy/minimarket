...
if (isset($_POST['batalkan'])) {
  $up = mysqli_query($koneksi, "update barang_pesan set status_po_batal=1, deskripsi_batal='" . $_POST['deskripsi'] . "' where id=" . $_GET['id'] . "");
  if ($up) {
    echo "<script>
      Swal.fire({
        customClass: {
        ...
        window.location.href = '?page=pembelian_alkes2';
      })
      </script>";
  } else {
    echo "<script>
