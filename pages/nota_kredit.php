...
if (isset($_GET['id_hapus'])) {
    $del2 = mysqli_query($koneksi, "delete from nota_kredit where id=".$_GET['id_hapus']."");
    if ($del2) {
        echo "<script type='text/javascript'>
		alert('Data Berhasil Di hapus !');
		window.location='index.php?page=nota_kredit'
		</script>";
    }
}
?>
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            <div class="box-footer">
              ...
              <a href="index.php?page=tambah_nota_kredit">
...
// membuka file JSON
$file = file_get_contents("http://localhost/ALKES/json/nota_kredit.php");
...
?>
  ...
  <tr>
    ...
    <td>
      <a href="index.php?page=nota_kredit&id_hapus=<?php echo $json[$i]['idd']; ?>" onclick="return confirm('Anda Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a>&nbsp;&nbsp;
      <a href="index.php?page=nota_kredit&id_ubah=<?php echo $json[$i]['idd']; ?>#openUbah"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a>&nbsp;&nbsp;
    </td>
  </tr>
  <?php } ?>
</table><br />
  ...
  <?php
  $da = mysqli_fetch_array(mysqli_query($koneksi, "select * from nota_kredit where id =$_GET[id_ubah]"));
  if (isset($_POST['ubah_riwayat'])) {
    $query = mysqli_query($koneksi,"UPDATE nota_kredit set tanggal='$_POST[tanggal2]',pelanggan_id='$_POST[pelanggan_id2]',deskripsi='$_POST[deskripsi2]',pilihan_biaya_id='$_POST[pilihan_biaya_id2]',harga='$_POST[harga2]' where id=$da[id]");
    if ($query) {
        echo "<script type='text/javascript'>
  ...
  <div id="openUbah" class="modalDialog">
     ...
     <form method="post">
              ...
              <select name="pelanggan_id2" class="form-control">
              <?php
              $query1 = mysqli_query($koneksi,"SELECT * FROM pelanggan");
              while ($row1 = mysqli_fetch_array($query1)){?>
              <option value="<?php echo $row1['id'];?>" <?php if ($row1['id'] == $da['pelanggan_id']) {echo "selected";}?>><?php echo $row1['nama_pelanggan'];?></option>
              <?php }?>
              </select>
