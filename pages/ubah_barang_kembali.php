<?php
$id=$_GET['id'];
$data=mysqli_fetch_array(mysqli_query($koneksi, "select *,barang_kembali.id as idd, barang_dikirim.id as id_brg_kirim from barang_gudang,barang_gudang_detail,barang_dijual,barang_dikirim,barang_kembali,pembeli,alamat_provinsi,alamat_kabupaten,alamat_kecamatan,alamat_kelurahan where barang_gudang.id=barang_gudang_detail.barang_gudang_id and barang_gudang_detail.id=barang_dijual.barang_gudang_detail_id and barang_dijual.id=barang_dikirim.barang_dijual_id and pembeli.id=barang_dijual.pembeli_id and alamat_provinsi.id=pembeli.provinsi_id and alamat_kabupaten.id=pembeli.kabupaten_id and alamat_kecamatan.id=pembeli.kecamatan_id and barang_dikirim.id=barang_kembali.barang_dikirim_id and barang_kembali.id=$id"));

if (isset($_POST['tambah_spk_masuk'])) {
	$ubah = mysqli_query($koneksi, "update barang_dikirim,barang_kembali set status_barang_kembali=0 where barang_kembali.id=".$_GET['id']." and barang_dikirim.id=barang_kembali.barang_dikirim_id");
	if ($ubah) {
	$query_spk=mysqli_query($koneksi, "update barang_kembali set barang_dikirim_id='".$_POST['id_barang']."', no_retur='".$_POST['nomor_retur']."', tgl_retur='".$_POST['tgl_retur']."', no_po_id='".$_POST['nomor_po']."' where id=".$_GET['id']."");
	mysqli_query($koneksi, "update barang_dikirim set status_barang_kembali=1 where id=".$_POST['id_barang']."");
	if ($query_spk) {
...
<div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><a href="index.php?page=barang_masuk">Ubah Retur Pengembalian</a></li></ol></section>
        ...
        <section class="col-lg-6 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            ...
            <div class="box-header with-border">
              <h3 class="box-title">Nomor Retur</h3>
            </div>
              <div class="box-body">
              <input name="nomor_retur" type="text" class="form-control" autofocus="autofocus" value="<?php echo $data['no_retur']; ?>" required="required"/>                                  
              </div>
            <div class="box-header with-border">
              <h3 class="box-title">Tanggal Retur</h3>
            </div>
              <div class="box-body">
              <input name="tgl_retur" type="date" class="form-control" value="<?php echo $data['tgl_retur']; ?>" required="required"/>                                  
              </div>
              <div class="box-header with-border">
              <h3 class="box-title">No PO/ID</h3>
            </div>
              <div class="box-body">
              <input name="nomor_po" type="text" class="form-control" value="<?php echo $data['no_po_id']; ?>" required="required"/>                                  
              </div>
            <div class="box-header with-border">
              <h3 class="box-title">Pilih Alkes</h3>
            </div>
              <div class="box-body">
                <select name="id_barang" id="id_barang" onchange="changeValue(this.value)" required="required" class="form-control">
                  <option value="<?php echo $data['id_brg_kirim']; ?>"><?php echo $data['nama_brg']; ?></option>
                  <?php 
					$result = mysqli_query($koneksi, "select *,barang_dikirim.id as idd from barang_dikirim, barang_dijual, barang_gudang, pembeli, alamat_provinsi, alamat_kabupaten, alamat_kecamatan where barang_gudang.id=barang_dijual.barang_gudang_id and barang_dijual.id=barang_dikirim.barang_dijual_id and pembeli.id=barang_dijual.pembeli_id and alamat_provinsi.id=pembeli.provinsi_id and alamat_kabupaten.id=pembeli.kabupaten_id and alamat_kecamatan.id=pembeli.kecamatan_id and tgl_sampai!='0000-00-00' and status_barang_kembali=0 order by nama_brg ASC");    
					$jsArray = "var dtBrg = new Array();
";        
					while ($row = mysqli_fetch_array($result)) {    ?>
                  <option value="<?php echo $row['idd']; ?>"><?php echo $row['nama_brg']; ?></option>
                  <?php 
					echo   
