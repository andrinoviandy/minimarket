...
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_gudang_set_1 where id=".$_GET['id_hapus']."");
	if ($del) {
		
		$upd = mysqli_query($koneksi, "update barang_gudang_po_set set jumlah_set=jumlah_set-1 where id=".$_GET['detail']."");
		
		if ($upd) {
		echo "<script type='text/javascript'>
		window.location='index.php?page=ubah_barang_set2&id=$_GET[id]&detail=$_GET[detail]'
		</script>";
		}
		}
	}
...
<div class="content-wrapper">
        ...
        <section class="col-lg-8 connectedSortable">
          ...
          <div class="box box-success"><!-- /.chat -->
            ...
            <div class="box-header with-border">
              ...
              <a href="index.php?page=ubah_barang_set2&id=<?php echo $_GET['id']; ?>#openPilihan"><button name="tambah_detail" class="btn btn-success" type="submit"><span class="fa fa-edit"></span> Tambah Stok</button></a>
              <?php } ?>
  ...
  <?php
  $q = mysqli_query($koneksi, "select * from barang_gudang_po_set where id=".$_GET['detail']."");
  $no=0;
...
<table  width="100%" id="example1" class="table table-bordered table-hover">
              <thead>
  <tr>
    <th><strong>Set Ke-</strong></th>
    <th>ID Set</th>
    <th><strong>Aksi</strong></th>
  </tr>
  </thead>
  <?php
  $q = mysqli_query($koneksi, "select * from barang_gudang_set_1 where barang_gudang_po_set_id=".$_GET['detail']."");
  $no=0;
  while ($d = mysqli_fetch_array($q)) {
  $no++;
  ?>
  <tr>
    ...
    <td>
    <a href="index.php?page=ubah_barang_set3&id=<?php echo $_GET['id']; ?>&detail=<?php echo $_GET['detail']; ?>&detail2=<?php echo $d['id']; ?>"><span data-toggle="tooltip" title="Ubah" class="fa fa-edit"></span></a>&nbsp;
    <?php if (isset($_SESSION['user_administrator'])) { ?>
    <a href="index.php?page=ubah_barang_set2&id=<?php echo $_GET['id']; ?>&id_hapus=<?php echo $d['id']; ?>&detail=<?php echo $_GET['detail']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="fa fa-trash"></span></a>
    <?php } ?>
    <!--&nbsp;
