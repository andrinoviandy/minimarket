...
<div class="content-wrapper">
            ...
            <div class="box-footer">
              ...
              <table width="100%" class="table no-border">
                ...
                <tr style="background-color:#0CC">
				  ...
				  if (isset($_POST['lihat'])) {
					  if ($_POST['akun']=='all') {
						$piutang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where tgl_bayar='".$_POST['lihat_tanggal']."' and u_p='Piutang'"));  
						  }
					  else {
                ...
                <tr style="background-color:#F30">
				  ...
				  if (isset($_POST['lihat'])) {
					  if ($_POST['akun']=='all') {
							$utang = mysqli_fetch_array(mysqli_query($koneksi,"select sum(nominal) as jumlah from utang_piutang_bayar where tgl_bayar='".$_POST['lihat_tanggal']."' and u_p='Hutang'"));	  
						  }
					  else {
  ...
  if (isset($_POST['lihat'])) {
	  if ($_POST['akun']=='all') {
		$query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and buku_kas.id=utang_piutang_bayar.buku_kas_id and tgl_bayar='".$_POST['lihat_tanggal']."' and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");  
		  }
	  else {
	  $query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and buku_kas.id=utang_piutang_bayar.buku_kas_id and tgl_bayar='".$_POST['lihat_tanggal']."' and buku_kas.id=".$_POST['akun']." and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");
	  }
	  }
  else {
	  $query = mysqli_query($koneksi, "select *,utang_piutang.nominal as nominal_up,utang_piutang.id as id_up from utang_piutang_bayar,buku_kas,utang_piutang,kategori_buku_kas where utang_piutang.id=utang_piutang_bayar.utang_piutang_id and kategori_buku_kas.id=utang_piutang.kategori_buku_kas_id and tgl_bayar='".date('Y-m-d')."' and buku_kas.id=utang_piutang_bayar.buku_kas_id and utang_piutang.u_p='Piutang' group by utang_piutang_id order by utang_piutang.tgl_input DESC");
  }
  $no=0;
