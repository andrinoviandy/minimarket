<?php 
$query = mysqli_query($koneksi, "select *,principle.id as id_principle from barang_pesan_inventory,principle where principle.id=barang_pesan_inventory.principle_id and barang_pesan_inventory.id='".$_GET['id']."'");
$data = mysqli_fetch_array($query);

if (isset($_POST['simpan_barang'])) {
	$s=mysqli_query($koneksi, "update utang_piutang_inventory set nominal=".$_POST['dalam_rupiah']." where no_faktur_no_po_inven='".$data['no_po_pesan']."'");
	$simpan = mysqli_query($koneksi, "update barang_pesan_inventory set total_price='".$_POST['total_price']."', total_price_ppn='".$_POST['total_price_ppn']."', cost_byair='".$_POST['cost_byair']."', cost_cf='".$_POST['cost_cf']."',nilai_tukar='".$_POST['nilai_tukar']."' where id=$_GET[id]");		
	if ($simpan) {
	echo "<script type='text/javascript'>
	alert('Data Berhasil Di Simpan !');
	window.location='index.php?page=ubah_pembelian_inventory2&id=$_GET[id]'</script>";
		}
	}

if (isset($_POST['simpan_tambah_aksesoris'])) {
	
	$simpan = mysqli_query($koneksi, "insert into barang_pesan_inventory_detail values('','".$_GET['id']."','".$_POST['id_akse']."','".$_POST['qty']."','".$data['mata_uang_id']."','".$_POST['harga_perunit']."','".$_POST['diskon']."','".$_POST['total_harga']."','".$_POST['catatan']."','')");
	mysqli_query($koneksi, "update barang_pesan_inventory set cost_byair=0, cost_cf=0 where id=$_GET[id]");
	echo "<script>
alert('Berhasil Di Simpan ! Harap isi kembali Ongkir nya !');	window.location='index.php?page=tambah_po_inventory2&id=$_GET[id]'</script>";
	}
	
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_pesan_inventory_detail where id=".$_GET['id_hapus']."");
	mysqli_query($koneksi, "update barang_pesan_inventory set cost_byair=0, cost_cf=0 where id=$_GET[id]");
	echo "<script type='text/javascript'>
	alert('Harap isi kembali Ongkir nya !');	window.location='index.php?page=tambah_po_inventory2&id=$_GET[id]';
		</script>";
	}
?>
...
<div class="content-wrapper">
  ...
  <thead>
    <tr>
      ...
      </td><td align="center" valign="bottom"><strong>Mata Uang     
      </strong>
      </td><td align="center" valign="bottom"><strong>Harga Per Unit </strong>      
      </td><td align="center" valign="bottom"><strong>Diskon (%)</strong>      
      </td><td align="center" valign="bottom"><strong>Total Harga
      </strong></td>
      <td align="center" valign="bottom"><strong>Catatan</strong>      
      </td><td align="center" valign="bottom"><strong>Aksi</strong></td></tr>
  </thead>
  
  <tr>
    <td>#</td>
    <form method="post" enctype="multipart/form-data">
    <td><select name="id_akse" id="id_akse" class="form-control select2" autofocus="autofocus" required onchange="changeValue(this.value)">
    	<option value="">-- Pilih Barang --</option>
        <?php 
		$q = mysqli_query($koneksi, "select * from barang_inventory order by nama_brg ASC");
...
";
    ...
    <td align="center"><textarea name="catatan_spek" class="form-control" placeholder="Catatan"></textarea></td>
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_inventory_detail.id as idd from barang_pesan_inventory_detail,mata_uang,barang_inventory where mata_uang.id=barang_pesan_inventory_detail.mata_uang_id and barang_inventory.id=barang_pesan_inventory_detail.barang_inventory_id and barang_pesan_inventory_detail.barang_pesan_inventory_id=$_GET[id]");
  ...
  <?php }} else {
      ...
      <form method="post">
    <tr>
    <td colspan="6" align="right" valign="bottom"><strong>Total Price =</strong></td>
    <td align="left">
      <?php
    $total = mysqli_fetch_array(mysqli_query($koneksi, "select *,sum(harga_total) as total from barang_pesan_inventory_detail where barang_pesan_inventory_id=$_GET[id]"));
    ...
    <tr>
    <td colspan="6" align="right" valign="bottom"><strong>Freight Cost =</strong></td>
    ...
    <tr>
    <td height="24" colspan="6" align="right" valign="bottom"><strong>Total Keseluruhan =</strong></td>
    ...
    <tr>
      <td height="24" colspan="6" align="right" valign="bottom">Nilai Tukar (satuan dalam rupiah) =</td>
    ...
    <tr>
      <td height="24" colspan="6" align="right" valign="bottom"><strong>Total Keseluruhan</strong> (Rupiah) =</td>
      <td colspan="2" align="left" valign="top"><?php
      $mu= mysqli_fetch_array(mysqli_query($koneksi, " select * from utang_piutang_inventory where no_faktur_no_po_inven='".$data['no_po_pesan']."'"));
	  if ($mu['nominal']!=0) {
...
<a href="index.php?page=tambah_po_alkes&simpan_barang=1&id=<?php echo $_GET['id']; ?>"><button name="simpan_barang" class="btn btn-success" type="submit"><span class="fa fa-check"></span> Simpan</button></a><?php } ?></center>
</form>
</div>
              </div>
            </div>
  ...
  if (isset($_POST['tambah_laporan'])) {
	$Result = mysqli_query($koneksi, "update barang_pesan_inventory_detail set qty=".$_POST['qty2'].", harga_perunit=".$_POST['harga_perunit2'].", diskon=".$_POST['diskon2'].", harga_total=".$_POST['total_harga2']." where id=".$_GET['id_ubah']."");
	if ($Result) {
		mysqli_query($koneksi, "update barang_pesan_inventory set cost_byair=0, cost_cf=0 where id=$_GET[id]");
		echo "<script type='text/javascript'>
	alert('Harap isi kembali Ongkir nya !');	window.location='index.php?page=tambah_po_inventory2&id=$_GET[id]';
  ...
  <div id="open_ubah" class="modalDialog">
     <div>
        ...
        <?php
        $da = mysqli_fetch_array(mysqli_query($koneksi, "select * from barang_pesan_inventory_detail where id=".$_GET['id_ubah'].""));
		?>
     <form method="post">
     <label>Qty</label>
