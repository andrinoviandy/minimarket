...
$insert_princ = mysqli_query($koneksi, "insert into principle values('','".$_SESSION['nama_princ']."','".str_replace("\n","<br>",$_SESSION['alamat_princ'])."','".$_SESSION['telp_princ']."','".$_SESSION['fax_princ']."','".$_SESSION['attn_princ']."')");
	...
	$q2=mysqli_query($koneksi, "select * from barang_pesan_detail_set_hash where akun_id=".$_SESSION['id']."");
	while ($d2 = mysqli_fetch_array($q2)) {
		
		$simpan2=mysqli_query($koneksi, "insert into barang_pesan_detail_set values('','$id_pesan','".$d2['barang_gudang_set_id']."','".$d2['qty']."','".$d2['mata_uang_id']."','".$d2['harga_perunit']."','".$d2['diskon']."','".$d2['harga_total']."','".$d2['catatan_spek']."','0')");
	...
	window.location='index.php?page=pembelian_alkes_set'</script>";
		}
	}

if (isset($_POST['simpan_tambah_aksesoris'])) {
	//$no=mysqli_num_rows(mysqli_query($koneksi, "select * from barang_pesan_hash"))+1;
	$simpan = mysqli_query($koneksi, "insert into barang_pesan_detail_set_hash values('','".$_SESSION['id']."','".$_POST['id_akse']."','".$_POST['qty']."','".$_POST['mata_uang']."','".$_POST['harga_perunit']."','".$_POST['diskon']."','".$_POST['total_harga']."','".$_POST['catatan_spek']."')");
	echo "<script>window.location='index.php?page=simpan_tambah_pemesanan_alkes_set'</script>";
	}
	
if (isset($_GET['id_hapus'])) {
	$del = mysqli_query($koneksi, "delete from barang_pesan_detail_set_hash where id=".$_GET['id_hapus']."");
	}
?>
...
";
    ...
    <?php 
	$q_uang=mysqli_query($koneksi, "select * from mata_uang where id=".$_SESSION['mata_uang']." order by id ASC");
  ...
  $no=0;
  $q_akse = mysqli_query($koneksi, "select *,barang_pesan_detail_set_hash.id as idd,barang_gudang_set.id as id_gudang from barang_pesan_detail_set_hash,barang_gudang_set,mata_uang where barang_gudang_set.id=barang_pesan_detail_set_hash.barang_gudang_set_id and mata_uang.id=barang_pesan_detail_set_hash.mata_uang_id and akun_id=".$_SESSION['id']."");
  ...
  <tr>
    ...
    <td align="center"><a href="index.php?page=simpan_tambah_pemesanan_alkes2_set&id_hapus=<?php echo $data_akse['idd']; ?>" onclick="return confirm('Yakin Akan Menghapus Item Ini ?')"><span data-toggle="tooltip" title="Hapus" class="ion-android-delete"></span></a></td>
    </tr>
  <?php }} else {
    ...
    <td align="center">
    <?php
    $total = mysqli_fetch_array(mysqli_query($koneksi, "select sum(harga_total) as total from barang_pesan_detail_set_hash"));
...
function yesnoCheck() {
    if (document.getElementById('yesCheck').checked) {
        document.getElementById('ifYes').style.display = 'block';
              ...
              <label>Nominal</label>
              <input id="" name="nominal" class="form-control" type="text" placeholder="Dalam Rupiah" value="" required="required"><br />
              <label>Klien</label>
