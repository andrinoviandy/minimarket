...
if (isset($_GET['start'])) {
    if (isset($_GET['tgl1']) && isset($_GET['tgl2'])) {
        if (isset($_GET['cari'])) {
            ...
            $sql = "select barang_dijual.*,barang_dijual.id as idd from barang_dijual where status_awal=1 and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]' group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC LIMIT $limit OFFSET $start";
        }
    } else {
        if (isset($_GET['cari'])) {
            ...
            $sql = "select barang_dijual.*,barang_dijual.id as idd from barang_dijual where status_awal=1 group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC LIMIT $limit OFFSET $start";
        }
    }
} else {
    //untuk jumlah
    if ($_GET['tgl1'] && $_GET['tgl2']) {
        if (isset($_GET['cari'])) {
            $sql = "select * from barang_dijual,barang_dijual_qty,barang_gudang,pembeli where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.status_awal=1 and (no_po_jual like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%' or nama_brg like '%$_GET[cari]%' or tipe_brg like '%$_GET[cari]%' or marketing like '%$_GET[cari]%' or subdis like '%$_GET[cari]%') and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]' group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC";
        } else {
            $sql = "select * from barang_dijual where status_awal=1 and tgl_jual between '$_GET[tgl1]' and '$_GET[tgl2]' group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC";
        }
    } else {
        if (isset($_GET['cari'])) {
            $sql = "select * from barang_dijual,barang_dijual_qty,barang_gudang,pembeli where barang_dijual.id=barang_dijual_qty.barang_dijual_id and barang_gudang.id=barang_dijual_qty.barang_gudang_id and pembeli.id=barang_dijual.pembeli_id and barang_dijual.status_awal=1 and (no_po_jual like '%$_GET[cari]%' or nama_pembeli like '%$_GET[cari]%' or nama_brg like '%$_GET[cari]%' or tipe_brg like '%$_GET[cari]%' or marketing like '%$_GET[cari]%' or subdis like '%$_GET[cari]%') group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC";
        } else {
            $sql = "select * from barang_dijual where status_awal=1 group by no_po_jual order by tgl_jual DESC, barang_dijual.id DESC";
        }
    }
}
